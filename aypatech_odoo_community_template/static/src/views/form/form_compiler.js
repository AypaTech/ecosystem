...
/** Lay out the chatter below the sheet or as a resizable aside per user preference. */
patch(FormCompiler.prototype, {
    /**
     * Rework the compiled form template to position the chatter, adding either
     * the bottom-sheet clone or the side resize handle.
     * @returns {Element} the adjusted compiled template
     */
    compile(node, params) {
        ...
        if (session.chatter_position === 'bottom') {
            ...
            setAttributes(chatterContainerHookXml, {
                't-att-style':
                    '__comp__.chatterState.width ? `width: ${__comp__.chatterState.width}px; max-width: ${__comp__.chatterState.width}px;` : ""',
            });
            const chatterContainerResizeHookXml = createElement('span');
            chatterContainerResizeHookXml.classList.add('apt_chatter_resize');
