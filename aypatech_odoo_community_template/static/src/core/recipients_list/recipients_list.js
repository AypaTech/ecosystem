...
import { RecipientsListPopover } from '@aypatech_odoo_community_template/core/recipients_popover/recipients_popover';
...
export class RecipientsList extends Component {
    ...
    get recipients() {
        if (this.props.internalOnly === true) {
            return [...this.props.thread.followers].filter(
                (follower) => follower.partner?.isInternalUser,
            );
        }
    ...
    getRecipientsSummary() {
        const items = this.recipients.slice(0, 3).map(({ partner }) => {
            const email = partner.email || '';
            const title = email || _t('no email address');
            const emailWithoutDomain = email.includes('@')
                ? email.split('@', 1)[0]
                : email;
            const text = email ? emailWithoutDomain : partner.name;
