...
export class AppsMenuWidgets extends Component {
    ...
    async _probeTickets() {
        try {
            ...
            this.state.ticketsCount = count;
        } catch {
            this.state.ticketsCount = false;
        }
    }
    ...
    get unreadCount() {
        // 18 has no store.globalCounter, mirror the messaging menu counter
        const failures = this.store.failures.reduce((acc, f) => acc + f.notifications.length, 0);
        return (this.store.inbox?.counter || 0) + failures;
    }
