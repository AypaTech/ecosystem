...
class AISession(models.Model):
    ...
    def init(self) -> None:
        ...
        create_index(
            self.env.cr,
            f'{self._table}_owner_recent_idx',
            self._table,
    ...
    def _turn_cost_limit(self) -> float:
        ...
        raw = self.env['ir.config_parameter'].sudo().get_param('aypatech_ai_assistant.ai_turn_cost_limit')
        try:
            value = float(raw)
        except (TypeError, ValueError):
            return 0.0
    ...
    def _retention_days(self) -> int:
        ...
        if not params.get_param('aypatech_ai_assistant.ai_session_retention_enabled'):
            return 0
        try:
            return max(int(params.get_param('aypatech_ai_assistant.ai_session_retention_days') or 0), 0)
        except ValueError:
            return 0
    ...
    def _client_action_timeout(self) -> int:
        ...
        raw = (
            ...
            .sudo()
            .get_param('aypatech_ai_assistant.ai_client_action_timeout', CLIENT_ACTION_TIMEOUT_SECONDS)
        )
        try:
            return int(raw)
        except (TypeError, ValueError):
    ...
    def _dispatch_inline(self) -> bool:
        ...
        if modules.module.current_test or config['workers']:
            return False
        mode = self.env['ir.config_parameter'].sudo().get_param('aypatech_ai_assistant.ai_dispatch_mode')
