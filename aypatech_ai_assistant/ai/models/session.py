...
class AISession(models.Model):
    ...
    share_user_ids = fields.Many2many(
        comodel_name='res.users',
        relation='aypatech_ai_session_share_user_rel',
        column1='session_id',
        column2='user_id',
        string='Shared With',
        help=(
    ...
    agent_id = fields.Many2one(
        ...
        string='Agent',
        default=lambda self: self.env['aypatech_ai.agent']._get_default(),
        ondelete='set null',
    )

    space_id = fields.Many2one(
        comodel_name='aypatech_ai.space',
        string='Space',
        help=(
    ...
    event_ids = fields.One2many(
        comodel_name='aypatech_ai.session.event',
        string='Events',
        readonly=True,
    ...
    pending_ids = fields.One2many(
        comodel_name='aypatech_ai.session.pending',
    ...
    attachment_ids = fields.One2many(
        ...
        inverse_name='res_id',
        domain=[('res_model', '=', 'aypatech_ai.session')],
        string='Attachments',
        copy=False,
        readonly=True,
    )
    ...
    turn_cost_spent = fields.Float(
        ...
        help=(
            ...
            '(aypatech_ai_assistant.ai_turn_cost_limit) the turn stops with an error.'
        ),
        readonly=True,
        default=0.0,
        copy=False,
    ...
    def _get_system_prompt(self) -> str:
        ...
        return self.env['aypatech_ai.agent']._get_default().system_prompt
    ...
    def _build_space_block(self) -> str:
        ...
        :meth:`~odoo.addons.aypatech_ai_assistant.ai.models.prompt_mixin.AIPromptMixin._render_prompt`
    ...
    def _render_system_prompt(self, raw: str) -> str:
        ...
        agent = self.agent_id or self.env['aypatech_ai.agent']._get_default()
    ...
    def _effective_model_record(self) -> models.BaseModel:
        ...
        return self.env['aypatech_ai.provider']._get_default().default_model_id
    ...
    def _effective_provider(self) -> models.BaseModel:
        ...
        return self.env['aypatech_ai.provider']._get_default()
    ...
    def _int_config_param(self, key: str, default: int) -> int:
        ...
        value = self.env['ir.config_parameter'].sudo().get_int(key, 0)
        return value if value > 0 else default

    @api.model
    def _max_iterations(self) -> int:
        """Return the configured maximum LLM rounds per worker slice."""
        return self._int_config_param('aypatech_ai_assistant.ai_max_iterations', MAX_ITERATIONS)
    ...
    def _slice_wallclock_seconds(self) -> int:
        ...
        configured = self._int_config_param(
            'aypatech_ai_assistant.ai_slice_wallclock_seconds', MAX_WALLCLOCK_SECONDS
        )
    ...
    def _turn_wallclock_seconds(self) -> int:
        ...
        return self._int_config_param(
            'aypatech_ai_assistant.ai_turn_wallclock_seconds', TURN_WALLCLOCK_SECONDS
        )

    @api.model
    def _turn_cost_limit(self) -> float:
        """Return the configured per-turn cost limit, or ``0.0`` when unset."""
        value = self.env['ir.config_parameter'].sudo().get_float('aypatech_ai_assistant.ai_turn_cost_limit', 0.0)
    ...
    def _get_filtered_catalog(self) -> list[dict]:
        ...
        catalog = list(tool_env['aypatech_mcp.tool'].sudo().get_tools(registry='odoo'))
        if self.agent_id:
    ...
    def _tool_client_kind(self, name: str) -> str | None:
        ...
        """
        for entry in self.env['aypatech_mcp.tool'].sudo().get_tools(registry='odoo'):
    ...
    def _get_essential_tool_names(self) -> list[str]:
        ...
        if self.agent_id:
            ...
            names = list(self.env['aypatech_ai.agent']._get_default_essential_tool_names())
        names.extend(
            entry['name']
            for entry in self._get_filtered_catalog()
            if entry.get('name')
    ...
    def _notify_share_change(self, previous: models.BaseModel) -> None:
        ...
        for partner in previous.partner_id - self._audience_partners():
            self.env['bus.bus']._sendone(
                partner,
                'aypatech_ai_assistant.ai_session_state',
    ...
    def _publish_event(self, event_type: str, payload: dict) -> None:
        ...
        self._bus_send(
            'aypatech_ai_assistant.ai_event',
            {
                'session_id': self.id,
                'type': event_type,
                'payload': payload,
    ...
    def _notify_state_transition(self, payload: dict) -> None:
        ...
        if new_state == 'done' and (
            self.pending_ids or self.env.context.get('aypatech_ai_skip_done_notification')
        ):
            return
        if new_state in ('done', 'waiting', 'error'):
            ...
            with suppress(Exception):
                self.env['bus.bus']._sendone(
                    ...
                    'aypatech_ai_assistant.ai_session_notification',
                    {
                        'session_id': self.id,
    ...
    def _post_inbox_notification(self, title: str, message: str) -> None:
        ...
        with suppress(Exception):
            ...
            if mail_message:
                mail_message.sudo().aypatech_ai_session_id = self.id
    ...
    def _append_event(self, entry: dict) -> models.BaseModel:
        ...
        event = self.env['aypatech_ai.session.event'].sudo()
        for _attempt in range(5):
            self.env.cr.execute(
                SQL(
                    'SELECT COALESCE(MAX(sequence), -1) + 1 '
                    'FROM aypatech_ai_session_event WHERE session_id = %s',
                    self.id,
                )
            )
            ...
            try:
                with self.env.cr.savepoint():
                    event = (
                        self.env['aypatech_ai.session.event']
                        .sudo()
                        .create(
                            {
                                'session_id': self.id,
    ...
    def _resolve_attachments(
        ...
        if new := attachments - self.attachment_ids:
            if new.filtered(lambda a: a.res_model and a.res_model != 'aypatech_ai.session'):
                raise UserError(_('One or more attachments could not be found.'))
            new.check_access('write')
            new.sudo().write({'res_model': 'aypatech_ai.session', 'res_id': self.id})
    ...
    def _persist_synthetic_event(self, call: dict, result, status: str) -> None:
        ...
        self.env['aypatech_mcp.log'].sudo().create(
            {
                'method': 'tools/call',
    ...
    def _get_rate_limit(self) -> int:
        ...
        provider = self.env['aypatech_ai.provider']._get_default()
    ...
    def _tool_dispatch_context(self) -> dict:
        ...
        return {'aypatech_mcp_session_id': self.id}
    ...
    def _dispatch_tool_call(self, name: str, arguments: dict, call_id: str) -> tuple:
        ...
        try:
            ...
            text, _info = tool_env['aypatech_mcp.tool']._call(
    ...
    def _stream_provider_round(
        ...
        try:
            payload = provider._request_responses(
                ...
                cache_key=f'aypatech_ai.session:{self.id}',
            )
    ...
    def enqueue_message(
        ...
        self.flush_recordset()
        self.env.cr.execute(
            'SELECT state FROM aypatech_ai_session WHERE id = %s FOR UPDATE',
            [self.id],
        )
    ...
    def _check_approval_gate(self, name: str, arguments: dict) -> dict:
        ...
        risk = self.env['aypatech_ai.approval']._assess_risk(name, arguments)
        if not risk:
            return {'action': 'dispatch'}
    ...
    def _enter_waiting_approval(
        ...
        risk = gate['risk']
        preview = (
            self.env['aypatech_ai.approval']._build_preview(call['name'], call['arguments'])
            or {}
        )
        pending = {
            'kind': 'approval',
    ...
    def _record_approval_audit(
        ...
        self.env['aypatech_ai.approval'].sudo().create(
            {
                'session_id': self.id,
    ...
    def _maybe_auto_compact(self) -> bool:
        ...
        compact = (
            self.with_context(aypatech_ai_skip_done_notification=True)
            if resume_state
            else self
        )
    ...
    def dismiss_notifications(self) -> bool:
        ...
        if partner := self.env.user.partner_id:
            messages = (
                ...
                .search(
                    [
                        ('aypatech_ai_session_id', 'in', self.ids),
    ...
    def _notification_badge_payload(self, user: models.BaseModel) -> dict:
        ...
        return {
            'count': len(ids),
            'session_ids': ids,
            'space_unread': self.env['aypatech_ai.space']
            .with_user(user)
            .count_sessions(ids),
        }
    ...
    def _push_notification_badge(self, user: models.BaseModel) -> None:
        ...
        with suppress(Exception):
            self.env['bus.bus']._sendone(
                user.partner_id,
                'aypatech_ai_assistant.ai_notification_badge',
    ...
    def fork_at_event(self, event_id: int) -> int:
        ...
        if source_events:
            self.env['aypatech_ai.session.event'].sudo().create(
                [
                    {
                        'session_id': fork.id,
    ...
    def _retention_days(self) -> int:
        ...
        if not params.get_bool('aypatech_ai_assistant.ai_session_retention_enabled'):
            return 0
        return max(params.get_int('aypatech_ai_assistant.ai_session_retention_days', 0), 0)
    ...
    def _client_action_timeout(self) -> int:
        ...
        return (
            ...
            .sudo()
            .get_int('aypatech_ai_assistant.ai_client_action_timeout', CLIENT_ACTION_TIMEOUT_SECONDS)
        )
    ...
    def _find_pending_session_ids(self, limit: int | None = None) -> list[int]:
        ...
        self.env.cr.execute(
            SQL(
                """
            SELECT id FROM aypatech_ai_session
        ...
        with self.pool.cursor() as cr:
            ...
            if cr.fetchone()[0]:
                try:
                    ...
                    session_su = env_su['aypatech_ai.session'].browse(session_id)
                    if session_su.exists() and session_su.state in (
                        ...
                        session = env['aypatech_ai.session'].browse(session_id)
    ...
    def _mark_session_error(self, session_id: int, message: str) -> None:
        ...
        with self.pool.cursor() as cr:
            session = api.Environment(cr, SUPERUSER_ID, {})['aypatech_ai.session'].browse(
                session_id
            )
            if session.exists():
    ...
    def _dispatch_inline(self) -> bool:
        ...
        if modules.module.current_test or config['workers']:
            return False
        mode = self.env['ir.config_parameter'].sudo().get_str('aypatech_ai_assistant.ai_dispatch_mode')
    ...
    def _trigger_worker(self) -> None:
        ...
        if request and self._dispatch_inline():
            queued = getattr(request, 'aypatech_ai_dispatch_ids', ())
            request.aypatech_ai_dispatch_ids = tuple(dict.fromkeys(queued + tuple(self.ids)))
