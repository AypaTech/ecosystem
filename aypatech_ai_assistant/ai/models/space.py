...
class AISpace(models.Model):
    ...
    def _count_by_space(self, session_ids: list[int] | None = None) -> dict[int, int]:
        ...
        :param session_ids: restrict the count to these sessions, all of them
            when omitted
        """
        sessions = self.env['aypatech_ai.session']
    ...
    def action_open_sessions(self) -> dict:
        ...
        """
        self.ensure_one()
        action = self.env['ir.actions.actions']._for_xml_id('aypatech_ai_assistant.ai_action_ai_session')
    ...
    def _check_domain(self) -> None:
        ...
        for record in self.filtered('domain'):
            try:
                Domain(record._parsed_domain()).validate(self.env['aypatech_ai.session'])
    ...
    def write(self, vals: dict) -> bool:
        ...
        if 'user_id' in vals or 'domain' in vals:
            filed = (
                self.env['aypatech_ai.session'].sudo().search([('space_id', 'in', self.ids)])
            )
            filed.filtered(
                lambda session: (
