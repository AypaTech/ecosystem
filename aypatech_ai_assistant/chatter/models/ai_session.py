...
from odoo.addons.aypatech_ai_assistant.ai.tools import with_record_ctx
from odoo.addons.aypatech_ai_assistant.chatter.tools import (
    COMPOSE_INTERFACES,
...
class AISession(models.Model):
    ...
    def _refresh_mention_answer(self) -> None:
        ...
        with suppress(Exception):
            message._bus_channel()._bus_send_store(message, fields=['body'])
    ...
    def _compose_agent(self) -> models.BaseModel:
        ...
        """
        space = self.env.ref('aypatech_ai_assistant.chatter_space_writing', raise_if_not_found=False)
        if space and space.sudo().agent_id.active:
            return space.sudo().agent_id
        return self.env['aypatech_ai.agent']._get_default()
