...
from odoo.addons.aypatech_ai_assistant.mcp.tools.schema import to_strict_schema


def _build_method_index(env: Environment) -> dict[str, dict[str, Any]]:
    """Scan the ``aypatech_mcp.mixin`` MRO for ``@mcp_tool`` methods, keyed by tool name.

    :raise ValueError: when two methods declare the same tool name.
    """
    index = {}
    for anchor in ('aypatech_mcp.mixin',):
...
def _fetch_db_index(env: Environment) -> dict[str, dict[str, Any]]:
    """Load active ``aypatech_mcp.tool`` records into a name-keyed index."""
    index = {}
    records = (
        env['aypatech_mcp.tool']
        .sudo()
        .search_read(
            [('active', '=', True)],
            fields=[
...
def get_tool_index(
    ...
    method_index = getattr(env.registry, '_aypatech_mcp_method_cache', None)
    cached_key = getattr(env.registry, '_aypatech_mcp_method_cache_key', None)
    stale = method_index is None or (cached_key is not None and cached_key != cache_key)
    if stale:
        method_index = _build_method_index(env)
        env.registry._aypatech_mcp_method_cache = method_index
        env.registry._aypatech_mcp_method_cache_key = cache_key
...
def invalidate_registry_cache(env: Environment) -> None:
    ...
    if hasattr(env.registry, '_aypatech_mcp_method_cache'):
        del env.registry._aypatech_mcp_method_cache
    if hasattr(env.registry, '_aypatech_mcp_method_cache_key'):
        del env.registry._aypatech_mcp_method_cache_key
