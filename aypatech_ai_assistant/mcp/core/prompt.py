...
def _build_method_index(env: Environment) -> dict[str, dict[str, Any]]:
    """Scan the ``aypatech_mcp.mixin`` MRO for ``@mcp_prompt`` methods, keyed by prompt name.

    :raise ValueError: when two methods declare the same prompt name.
    """
    index = {}
    Model = env.registry.get('aypatech_mcp.mixin')
    if Model is None:
        return index
    seen = set()
...
def _fetch_db_index(env: Environment) -> dict[str, dict[str, Any]]:
    """Load active ``aypatech_mcp.prompt`` records into a name-keyed index."""
    index = {}
    if env.registry.get('aypatech_mcp.prompt') is None:
        return index
    records = (
        env['aypatech_mcp.prompt']
        .sudo()
        .search_read(
            [('active', '=', True)],
...
def get_prompt_index(env: Environment) -> dict[str, dict[str, Any]]:
    ...
    method_index = getattr(env.registry, '_aypatech_mcp_prompt_cache', None)
    cached_key = getattr(env.registry, '_aypatech_mcp_prompt_cache_key', None)
    stale = method_index is None or (cached_key is not None and cached_key != cache_key)
    if stale:
        method_index = _build_method_index(env)
        env.registry._aypatech_mcp_prompt_cache = method_index
        env.registry._aypatech_mcp_prompt_cache_key = cache_key
...
def invalidate_prompt_cache(env: Environment) -> None:
    ...
    if hasattr(env.registry, '_aypatech_mcp_prompt_cache'):
        del env.registry._aypatech_mcp_prompt_cache
    if hasattr(env.registry, '_aypatech_mcp_prompt_cache_key'):
        del env.registry._aypatech_mcp_prompt_cache_key
