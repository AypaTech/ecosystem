...
from odoo.addons.aypatech_ai_assistant.mcp.core.route import mcp_route
from odoo.addons.aypatech_ai_assistant.mcp.tools import common, protocol, version
from odoo.addons.aypatech_ai_assistant.mcp.tools.exception import (
    MCPResourceNotFound,
    MCPScopeDenied,
)
from odoo.addons.aypatech_ai_assistant.mcp.tools.version import ProtocolProfile
...
class MCPController(http.Controller):
    ...
    def _log_request(self, method: str, **kwargs: Any) -> None:
        ...
        if config.get('mcp_logging', True):
            ...
            request.env['aypatech_mcp.log'].log(
    ...
    def _get_allowed_origins(self) -> set[str]:
        ...
        configured = param.get_str('aypatech_ai_assistant.mcp_allowed_origins')
        origins = {entry.strip().rstrip('/') for entry in configured.split(',')}
        if base_url := param.get_str('web.base.url'):
    ...
    def _check_origin(self) -> Response | None:
        ...
        allow_any = (
            ...
            .sudo()
            .get_bool('aypatech_ai_assistant.mcp_allow_any_origin')
        )
        if allow_any:
            return None
        self._log_request(
    ...
    def _get_session(self, session_id: str | None) -> models.BaseModel | None:
        ...
        if (
            session := request.env['aypatech_mcp.session']
            .sudo()
            .search(
                [
    ...
    def _claim_notifications(self, session_id: int, after_id: int = 0) -> list[tuple]:
        ...
        """
        table = SQL.identifier('aypatech_mcp_notification')
    ...
    def _handle_initialize(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        session = (
            request.env['aypatech_mcp.session']
            .sudo()
            .create(
                {
                    'user_id': request.env.uid,
    ...
    def _handle_tools_list(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            'tools': request.env['aypatech_mcp.tool']
            .sudo()
            .get_tools(
                registry='mcp',
            ),
    ...
    def _handle_tools_call(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            result, _record_info = retrying(
                partial(
                    request.env['aypatech_mcp.tool']._call,
                    tool_name,
    ...
    def _handle_resources_read(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            entry = request.env['aypatech_mcp.mixin']._dispatch_resources_read(
                uri,
            )
    ...
    def _handle_prompts_list(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            'prompts': request.env['aypatech_mcp.prompt'].sudo().get_prompts(),
        }
    ...
    def _handle_prompts_get(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return request.env['aypatech_mcp.prompt'].get_prompt(
            params.get('name'),
    ...
    def _handle_completion_complete(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        return (
            request.env['aypatech_mcp.prompt']
            .sudo()
            .complete_argument(
                params.get('ref') or {},
    ...
    def mcp_get(self, **kw: Any) -> Response:
        ...
        if last_event_id := request.httprequest.headers.get('Last-Event-ID'):
            if resume := request.env['aypatech_mcp.notification'].search(
                [
