...
from odoo.addons.aypatech_ai_assistant.automation.tools.constants import MAX_PROMPT_CHARS
...
def post_session_event(session: models.BaseModel, kind: str, payload: dict) -> None:
    ...
    Event = session.env['aypatech_ai.session.event'].sudo()
    last = Event.search(
...
def _resolve_previous_session(
    ...
    if action.agent_chain_strategy != 'per_record':
        return action.env['aypatech_ai.session'].sudo().browse([])
    Session = action.env['aypatech_ai.session'].sudo()
...
def _build_prompt(
    ...
    Mixin = action.env['aypatech_ai_assistant.ai_prompt.mixin']
...
def _create_session(
    ...
    session = action.env['aypatech_ai.session'].sudo().with_user(user).create(vals)
    if render_err:
...
def fire_action(action: models.BaseModel, eval_context: dict) -> models.BaseModel:
    ...
    Session = action.env['aypatech_ai.session']
