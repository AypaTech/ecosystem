...
import { recordSkillUse } from '@aypatech_ai_assistant/skills/chat/recent_skills';
...
test('every visible skill is listed', async () => {
    reset();
    await mountPanel();
    expect(queryAllTexts('.ayp_skill_label')).toEqual(['Alpha', 'Beta']);
});

test('the search field filters the entries', async () => {
    reset();
    await mountPanel();
    await click('.ayp_skills_search input');
    await edit('bet');
    await animationFrame();
    expect(queryAllTexts('.ayp_skill_label')).toEqual(['Beta']);
});
...
test('a filter matching no skill renders the empty state', async () => {
    ...
    await animationFrame();
    expect('.ayp_skill').toHaveCount(0);
    expect(queryFirst('.ayp_skills_empty')).not.toBe(null);
});
...
test('arrow keys move the active entry and enter runs it', async () => {
    ...
    await animationFrame();
    expect(queryAll('.ayp_skill')[1]).toHaveClass('active');
    await press('Enter');
    await animationFrame();
    expect(picked).toEqual(['beta']);
});
...
test('the search field takes the caret when autofocus is asked for', async () => {
    ...
    expect('.ayp_skills_search input').toBeFocused();
});

test('without autofocus the panel itself takes focus, not the search field', async () => {
    reset();
    await mountPanel({ autofocus: false });
    expect('.ayp_skills_search input').not.toBeFocused();
    expect('.ayp_skills_panel').toBeFocused();
});
...
test('a recently used skill is listed first under its own group', async () => {
    ...
    recordSkillUse('beta');
    await mountPanel();
    expect(queryAllTexts('.ayp_skill_label')).toEqual(['Beta', 'Alpha']);
    expect(queryAllTexts('.ayp_skills_group')).toEqual(['Recently used', 'All skills']);
});
...
test('a withheld skill is listed apart, inert, and says what it needs', async () => {
    ...
    expect('.ayp_skill_locked').toHaveCount(1);
    expect('.ayp_skill_locked').toHaveAttribute('disabled');
    expect(queryFirst('.ayp_skill_locked .ayp_skill_preview')).toHaveText(
        'needs a record open',
    );
    expect(queryAll('.ayp_skills_group').at(-1)).toHaveText('Once something is open');
});
...
test('the empty state stays away while a withheld skill is listed', async () => {
    ...
    expect('.ayp_skills_empty').toHaveCount(0);
    expect('.ayp_skill_locked').toHaveCount(1);
});
