...
import { getRecentSkillNames, recordSkillUse } from '@aypatech_ai_assistant/skills/chat/recent_skills';
...
test('unreadable storage degrades to an empty list', () => {
    browser.localStorage.clear();
    recordSkillUse('alpha');
    browser.localStorage.setItem(`aypatech_ai_assistant.skills_recent.${user.userId}`, '{not json');
