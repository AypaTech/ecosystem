import { advanceTime, describe, expect, freezeTime, mockDate, test } from '@odoo/hoot';
import { animationFrame, queryAllTexts, queryText } from '@odoo/hoot-dom';
import {
    defineModels,
    fields,
    mockService,
    models,
    mountView,
    mountWithCleanup,
} from '@web/../tests/web_test_helpers';
import { defineMailModels } from '@mail/../tests/mail_test_helpers';

import { ScheduleCountdownField } from '@aypatech_ai_assistant/schedule/views/fields/schedule_countdown/schedule_countdown';

describe.current.tags('aypatech_ai_schedule');
defineMailModels();

const { DateTime } = luxon;

const NOW = '2026-03-01 12:00:00';

const ARCH = `
    <form>
        <field name="resume_at" widget="schedule_countdown"/>
    </form>`;

class AypaTechAiSession extends models.Model {
    _name = 'aypatech_ai.session';
    resume_at = fields.Datetime();
    _records = [
        { id: 1, resume_at: false },
        { id: 2, resume_at: '2026-03-01 11:00:00' },
        { id: 3, resume_at: '2026-03-01 12:00:30' },
        { id: 4, resume_at: '2026-03-01 12:05:30' },
    ];
}
defineModels([AypaTechAiSession]);

/**
 * Install an inert bus mock that records the channels the widget follows.
 * @returns {object} the channels added and deleted so far, in order
 */
function mockBus() {
    const channels = { added: [], deleted: [] };
    mockService('bus_service', {
        subscribe() {},
        unsubscribe() {},
        addChannel(channel) {
            channels.added.push(channel);
        },
        deleteChannel(channel) {
            channels.deleted.push(channel);
        },
    });
    return channels;
}

/**
 * Stop the clock dead at the fixture time.
 *
 * `mockDate` alone only pins the origin: hoot keeps adding the elapsed real
 * time on every read, so a target set an exact number of minutes away is
 * already a millisecond short when the widget floors it, and `2h 20m` renders
 * as `2h 19m`. Freezing makes the remaining distance exact.
 *
 * Only call this once the component is mounted -- a frozen clock turns
 * `setTimeout`, `setInterval` and `requestAnimationFrame` into no-ops, so a
 * mount awaiting an animation frame would never resolve.
 */
function freezeClock() {
    mockDate(NOW, 0);
    freezeTime();
}

/**
 * Pin the clock at the fixture time and render the widget for one record.
 *
 * The zone is pinned along with the date so that the absolute datetime the
 * widget renders does not depend on the zone the runner happens to carry.
 * @param {number} resId the session record to open
 * @returns {Promise<object>} the mounted form view and the recorded channels
 */
async function mountCountdown(resId) {
    mockDate(NOW, 0);
    const channels = mockBus();
    const view = await mountView({
        resModel: 'aypatech_ai.session',
        resId,
        type: 'form',
        arch: ARCH,
    });
    return { view, channels };
}

/**
 * Build the minimal record props the field reads.
 * @param {string} resModel the model the field is bound to
 * @param {string|false} isoUtc the UTC target time, or false for an empty value
 * @returns {object} the record stub
 */
function makeRecord(resModel, isoUtc) {
    return {
        resModel,
        resId: 42,
        data: {
            resume_at: isoUtc ? DateTime.fromISO(isoUtc, { zone: 'utc' }) : false,
        },
    };
}

/**
 * Mount the field on a record stub over a mocked bus.
 * @param {string} resModel the model the field is bound to
 * @param {string|false} isoUtc the UTC target time, or false for an empty value
 * @returns {Promise<object>} the mounted component and the recorded channels
 */
async function mountBareField(resModel, isoUtc) {
    mockDate(NOW, 0);
    const channels = mockBus();
    const field = await mountWithCleanup(ScheduleCountdownField, {
        props: { name: 'resume_at', record: makeRecord(resModel, isoUtc) },
    });
    return { field, channels };
}

test('an empty target renders the em dash', async () => {
    await mountCountdown(1);
    expect(queryText('.ayp_schedule_countdown')).toBe('—');
});

test('a past target renders the absolute datetime and the due label', async () => {
    await mountCountdown(2);
    const [absolute, due] = queryAllTexts('.ayp_schedule_countdown span');
    expect(absolute).toMatch(/2026/);
    expect(due).toBe('(due)');
});

test('the rendered countdown ticks down as the seconds elapse', async () => {
    await mountCountdown(4);
    expect(queryText('.ayp_schedule_countdown')).toMatch(/^5m \d+s$/);
    await advanceTime(60 * 1000);
    await animationFrame();
    expect(queryText('.ayp_schedule_countdown')).toMatch(/^4m \d+s$/);
});

test('the rendered countdown flips to the due label once the target passes', async () => {
    await mountCountdown(3);
    expect(queryText('.ayp_schedule_countdown')).toMatch(/^\d+s$/);
    await advanceTime(31 * 1000);
    await animationFrame();
    expect('.ayp_schedule_countdown_due').toHaveText('(due)');
});

test('countdownText renders seconds under a minute', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T12:00:30');
    freezeClock();
    expect(field.countdownText).toBe('30s');
});

test('countdownText renders minutes and seconds under an hour', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T12:05:30');
    freezeClock();
    expect(field.countdownText).toBe('5m 30s');
});

test('countdownText renders hours and minutes under a day', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T14:20:00');
    freezeClock();
    expect(field.countdownText).toBe('2h 20m');
});

test('countdownText renders days and hours beyond a day', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-04T15:00:00');
    freezeClock();
    expect(field.countdownText).toBe('3d 3h');
});

test('countdownText is empty for an empty target', async () => {
    const { field } = await mountBareField('aypatech_ai.session', false);
    expect(field.isEmpty).toBe(true);
    expect(field.countdownText).toBe('');
});

test('countdownText is empty for an elapsed target', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T11:00:00');
    freezeClock();
    expect(field.isPast).toBe(true);
    expect(field.countdownText).toBe('');
});

test('a running state event for this session forces a re-tick', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T13:00:00');
    const before = field.state.tick;
    field.busHandler({
        session_id: field.props.record.resId,
        type: 'state',
        payload: { state: 'running' },
    });
    expect(field.state.tick).toBe(before + 1);
});

test('a foreign, non-state, or non-running event never re-ticks', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T13:00:00');
    const before = field.state.tick;
    field.busHandler({ session_id: 999, type: 'state', payload: { state: 'running' } });
    field.busHandler({
        session_id: field.props.record.resId,
        type: 'text',
        payload: { state: 'running' },
    });
    field.busHandler({
        session_id: field.props.record.resId,
        type: 'state',
        payload: { state: 'done' },
    });
    field.busHandler(null);
    field.busHandler({ session_id: field.props.record.resId, type: 'state' });
    expect(field.state.tick).toBe(before);
});

test('the field only listens to the bus on the session model', async () => {
    const { field } = await mountBareField('aypatech_ai.session', '2026-03-01T13:00:00');
    expect(typeof field.busHandler).toBe('function');
    const other = await mountBareField('res.partner', '2026-03-01T13:00:00');
    expect(other.field.busHandler).toBe(null);
    expect(other.channels.added).not.toInclude('aypatech_ai_assistant.ai_session_42');
});

test('the field subscribes to the bound session channel', async () => {
    const { channels } = await mountBareField('aypatech_ai.session', '2026-03-01T13:00:00');
    expect(channels.added).toInclude('aypatech_ai_assistant.ai_session_42');
});

test('the form view subscribes to the channel of the session it opens', async () => {
    const { channels } = await mountCountdown(4);
    expect(channels.added).toInclude('aypatech_ai_assistant.ai_session_4');
});
