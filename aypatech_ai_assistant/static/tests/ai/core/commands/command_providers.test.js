...
import '@aypatech_ai_assistant/ai/core/commands/command_providers';

describe.current.tags('aypatech_ai');
patchTranslations();

function getProvider() {
    return registry.category('command_provider').get('aypatech_ai_sessions');
}
...
test('running a session command opens the chat via action service', async () => {
    ...
    expect(env.services.action.actions[0].tag).toBe('aypatech_ai_assistant.ai_chat');
