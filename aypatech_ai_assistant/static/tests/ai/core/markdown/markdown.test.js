...
import { renderMarkdown } from '@aypatech_ai_assistant/ai/core/markdown/markdown';
...
test('fenced block embeds the copy button', () => {
    const out = renderMarkdown('```\nhello\n```');
    expect(out).toMatch(/class="ayp_code_copy"/);
...
test('task list marker [ ] becomes unchecked checkbox input', () => {
    ...
    expect(out).toMatch(/ayp_md_task/);
...
test('images carry the ayp_md_image class the click-to-preview handler targets', () => {
    expect(renderMarkdown('![alt](https://example.com/p.png)')).toMatch(
        /class="ayp_md_image"/,
    );
    expect(renderMarkdown('![g](/web/image/1032)')).toMatch(/class="ayp_md_image"/);
});
...
test('a bare model,id reference becomes a backend record link', () => {
    ...
    expect(
        out.includes(
            '<a href="/odoo/res.partner/5" class="ayp_record_link" ' +
...
test('a reference already inside a link is not linked twice', () => {
    ...
    expect(out.includes('ayp_record_link')).toBe(false);
});

test('a reference inside code is left verbatim', () => {
    expect(renderMarkdown('use `res.partner,5` here').includes('ayp_record_link')).toBe(
        false,
    );
    expect(renderMarkdown('```\nres.partner,5\n```').includes('ayp_record_link')).toBe(
        false,
    );
});
...
test('an explicit link with a model,id target becomes a backend record link', () => {
    ...
    expect(
        out.includes(
            '<a href="/odoo/account.move/42" class="ayp_record_link" ' +
...
test('clicking ayp_code_copy button invokes navigator.clipboard.writeText', async () => {
    ...
    try {
        ...
        host.innerHTML =
            '<pre><button class="ayp_code_copy">Copy</button><code>hello()</code></pre>';
        document.body.appendChild(host);
        host.querySelector('.ayp_code_copy').click();
...
test('clicking outside ayp_code_copy does not trigger clipboard', async () => {
    const original = navigator.clipboard;
    const writes = [];
    Object.defineProperty(navigator, 'clipboard', {
        configurable: true,
...
test('a bare download url a tool returned becomes a clickable link', () => {
    ...
    expect(
        out.includes(
            '<a href="/web/content/45604?download=1" class="ayp_file_link" ' +
...
test('a download url already inside a markdown link is not linked twice', () => {
    ...
    expect(out.includes('ayp_file_link')).toBe(false);
});
...
test('text after a download url is preserved', () => {
    ...
    expect(out.includes('ayp_file_link')).toBe(true);
