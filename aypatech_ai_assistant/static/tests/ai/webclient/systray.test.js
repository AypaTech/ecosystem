...
import { AypaTechAISystray } from '@aypatech_ai_assistant/ai/webclient/systray/systray';
...
test('systray loads sessions and exposes runningCount for 2 of 3 sessions', async () => {
    ...
    const systray = await mountWithCleanup(AypaTechAISystray, { props: {} });
...
test('systray loads the notification badge count and renders it', async () => {
    onRpc('aypatech_ai.session', 'search_read', () => []);
    onRpc('aypatech_ai.session', 'notification_badge', () => ({
        count: 3,
        session_ids: [1, 2, 3],
    }));
    makeBusMock();
    ...
    expect('.ayp_systray_count').toHaveText('3');
    expect('.ayp_systray_badge .ayp_systray_dot').toHaveCount(0);
});
...
test('the notification badge updates live from the bus', async () => {
    ...
    bus.emit('aypatech_ai_assistant.ai_notification_badge', { count: 5, session_ids: [1, 2, 3, 4, 5] });
    expect(systray.badge.count).toBe(5);
    bus.emit('aypatech_ai_assistant.ai_notification_badge', { count: 0, session_ids: [] });
...
test('the badge falls back to the running dot when the count is zero', async () => {
    ...
    makeBusMock();
    makeChatWindowService();
    await mountWithCleanup(AypaTechAISystray, { props: {} });
    expect('.ayp_systray_count').toHaveCount(0);
    expect('.ayp_systray_badge .ayp_systray_dot.ayp_state_running').toHaveCount(1);
});
...
test('systray bus event updates a known session row in place', async () => {
    ...
    makeChatWindowService();
    const systray = await mountWithCleanup(AypaTechAISystray, { props: {} });
    bus.emit('aypatech_ai_assistant.ai_session_state', {
        session_id: 1,
        state: 'running',
        name: 'Renamed',
    });
...
test('systray bus event for unknown session triggers reload', async () => {
    ...
    bus.emit('aypatech_ai_assistant.ai_session_state', { session_id: 99, state: 'running' });
...
test('systray bus event removes a deleted session from the list', async () => {
    ...
    bus.emit('aypatech_ai_assistant.ai_session_state', { session_id: 1, deleted: true });
...
test('onNewChat creates a session and opens it via chat_window service', async () => {
    ...
    const seen = [];
    onRpc('aypatech_ai.session', 'search_read', () => []);
    onRpc('aypatech_ai.session', 'create', ({ args }) => {
        seen.push(args[0]);
        return [++nextId];
    });
    makeBusMock();
...
test('onOpenFullChat dispatches the chat action', async () => {
    ...
    expect(actions[0].tag).toBe('aypatech_ai_assistant.ai_chat');
});
