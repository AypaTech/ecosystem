...
test('openDocument sends an AI session to the chat client action', () => {
    ...
    instance.openDocument({ id: 7, model: 'aypatech_ai.session' });
...
test('an unread AI inbox message opens the chat and is marked done', () => {
    ...
    const doneCalls = [];
    const thread = { id: 7, model: 'aypatech_ai.session' };
    const msg = {
        needaction: true,
        setDone: () => doneCalls.push(actions.length),
    };
    instance.onClickThread(false, thread, msg);
    expect(actions).toEqual([CHAT_ACTION_7]);
    expect(doneCalls).toEqual([1]);
});

test('an unread non-AI thread still opens in Discuss', () => {
    const opened = [];
    const { instance, actions } = withActionService(MessagingMenu.prototype, {
        openDiscussion: (thread) => opened.push(thread),
    });
    const thread = { id: 3, model: 'discuss.channel' };
    instance.onClickThread(false, thread, { needaction: false });
    expect(actions).toEqual([]);
...
test('marking an AI thread as read does not open the chat', () => {
    const marked = [];
    const { instance, actions } = withActionService(MessagingMenu.prototype, {
        markAsRead: (thread) => marked.push(thread),
    });
    const thread = { id: 7, model: 'aypatech_ai.session' };
    instance.onClickThread(true, thread, { needaction: true, setDone: () => {} });
    expect(actions).toEqual([]);
    expect(marked).toEqual([thread]);
});
