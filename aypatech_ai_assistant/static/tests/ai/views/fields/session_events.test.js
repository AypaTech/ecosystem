...
import { SessionEventsField } from '@aypatech_ai_assistant/ai/views/fields/session_events/session_events';
...
class SessionStub extends models.Model {
    _name = 'aypatech_ai_assistant.ai_session_events_stub';
    name = fields.Char();
    log = fields.Json();
    _records = [
...
async function mountField(resId) {
    ...
    await mountView({
        resModel: 'aypatech_ai_assistant.ai_session_events_stub',
        resId,
        type: 'form',
        arch: ARCH,
    });
...
test('the field renders the whole transcript of a session log', async () => {
    ...
    expect(queryAllTexts('.ayp_bubble_body').join(' ')).toMatch(/no partner/);
    expect(queryAll('.ayp_bubble_body strong').length).toBeGreaterThan(0);
});
