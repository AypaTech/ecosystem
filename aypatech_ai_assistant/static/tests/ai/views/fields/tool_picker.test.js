...
import '@aypatech_ai_assistant/ai/views/fields/tool_picker/tool_picker';

describe.current.tags('aypatech_ai');
defineMailModels();
patchTranslations();

class AypaTechAiToolModel extends models.Model {
    _name = 'aypatech_ai_assistant.ai_tool_model';
...
defineModels([AypaTechAiToolModel]);
...
async function mountFormRecordingSaves(resId) {
    const saved = [];
    onRpc('aypatech_ai_assistant.ai_tool_model', 'web_save', ({ args }) => {
        saved.push(args[1]);
    });
    await mountView({ resModel: 'aypatech_ai_assistant.ai_tool_model', resId, type: 'form', arch: ARCH });
    return saved;
}

test('ToolPickerField renders tag chips for each selected name', async () => {
    await mountView({
        resModel: 'aypatech_ai_assistant.ai_tool_model',
        resId: 1,
        type: 'form',
        arch: ARCH,
    });
