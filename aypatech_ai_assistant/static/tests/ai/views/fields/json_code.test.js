...
import { JsonCodeField } from '@aypatech_ai_assistant/ai/views/fields/json_code/json_code';
...
class AypaTechAiModelStub extends models.Model {
    _name = 'aypatech_ai_assistant.model_stub';
    name = fields.Char();
    payload = fields.Json();
    _records = [
...
defineModels([AypaTechAiModelStub]);
...
async function mountFormRecordingSaves(resId = 1) {
    const saved = [];
    onRpc('aypatech_ai_assistant.model_stub', 'web_save', ({ args }) => {
        saved.push(args[1]);
    });
    await mountView({ resModel: 'aypatech_ai_assistant.model_stub', resId, type: 'form', arch: ARCH });
    return saved;
}

test('JsonCodeField renders CodeEditor with pretty-printed initial value', async () => {
    await mountView({
        resModel: 'aypatech_ai_assistant.model_stub',
        resId: 1,
        type: 'form',
        arch: ARCH,
    });
