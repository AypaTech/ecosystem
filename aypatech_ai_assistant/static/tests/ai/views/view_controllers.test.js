...
import '@aypatech_ai_assistant/ai/views/list/list_controller';
import '@aypatech_ai_assistant/ai/views/kanban/kanban_controller';
import '@aypatech_ai_assistant/ai/views/pivot/pivot_controller';
import '@aypatech_ai_assistant/ai/views/graph/graph_controller';

describe.current.tags('aypatech_ai');
defineMailModels();

class AypaTechAiViewModel extends models.Model {
    _name = 'aypatech_ai_assistant.ai_view_model';
    name = fields.Char();
...
defineModels([AypaTechAiViewModel]);
...
test('list controller dispatches a list view_context on mount', async () => {
    ...
    await mountView({
        resModel: 'aypatech_ai_assistant.ai_view_model',
        type: 'list',
        arch: `<list><field name="name"/><field name="amount"/></list>`,
    });
    expect(captured.length).toBeGreaterThan(0);
    expect(captured[0].kind).toBe('list');
    expect(captured[0].model).toBe('aypatech_ai_assistant.ai_view_model');
