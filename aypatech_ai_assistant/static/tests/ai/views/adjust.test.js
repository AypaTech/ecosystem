...
import { applyAdjustSearch } from '@aypatech_ai_assistant/ai/views/adjust';
...
class AypaTechAiAdjustModel extends models.Model {
    _name = 'aypatech_ai_assistant.ai_adjust_model';
    name = fields.Char();
...
defineModels([AypaTechAiAdjustModel]);
...
function mockChatWindow(activeSessionId) {
    ...
    onRpc('aypatech_ai.session', 'set_view_context', () => ({}));
}

async function mountAdjustList() {
    await mountView({
        resModel: 'aypatech_ai_assistant.ai_adjust_model',
        type: 'list',
...
test('applies filters, group bys and field searches to the live view', async () => {
    ...
    expect(result.model).toBe('aypatech_ai_assistant.ai_adjust_model');
