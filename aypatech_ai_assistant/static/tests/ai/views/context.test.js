...
import {
    ...
    probeCurrentView,
    seedSessionContext,
} from '@aypatech_ai_assistant/ai/views/context';
...
function makeController({
    ...
    const env = {
        services: {
            orm,
            'aypatech_ai_assistant.ai_chat_window': {
                get sessionIds() {
                    return sessionIds;
                },
            },
...
test('captureViewContext dispatches set_view_context with silent orm if available', async () => {
    ...
    expect(calls[0][1]).toBe('aypatech_ai.session');
...
test('makeListContextDispatch builds payload with view_type + domain and only fires on change', async () => {
    ...
    expect(model).toBe('aypatech_ai.session');
