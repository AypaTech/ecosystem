...
import { PromptHistoryDialog } from '@aypatech_ai_assistant/ai/components/prompt_history_dialog/prompt_history_dialog';
...
function mockHistory(entries, diffByIndex = {}) {
    const asked = [];
    onRpc('aypatech_ai.agent', 'read', () => [
        { id: 3, prompt_history_metadata: { system_prompt: entries } },
    ]);
    onRpc('aypatech_ai.agent', 'prompt_history_unified_diff', ({ args }) => {
        asked.push(args);
...
test('the rendered diff tags added, removed and context lines', async () => {
    ...
    await mountDialog();
    expect(queryAllTexts('.ayp_prompt_history_added')).toEqual(['+add this line']);
    expect(queryAllTexts('.ayp_prompt_history_removed')).toEqual(['-drop this line']);
    expect(queryAllTexts('.ayp_prompt_history_hunk')).toEqual(['@@ -1,3 +1,3 @@']);
});
...
test('a record without history metadata at all is handled', async () => {
    onRpc('aypatech_ai.agent', 'read', () => [{ id: 3 }]);
    const dialog = await mountDialog();
    expect(dialog.state.entries).toEqual([]);
    expect(dialog.state.selectedIndex).toBe(null);
});
...
test('restoring a revision dispatches the returned action and closes', async () => {
    ...
    onRpc('aypatech_ai.agent', 'prompt_history_restore', ({ args }) => {
        restoreArgs = args;
        return { type: 'ir.actions.act_window', res_model: 'aypatech_ai.agent' };
    });
    const dispatched = [];
    mockService('action', {
...
test('restoring closes without dispatching when the server returns nothing', async () => {
    mockHistory([{ create_date: '2026-07-01 08:00:00', create_user_name: 'Ann' }]);
    onRpc('aypatech_ai.agent', 'prompt_history_restore', () => false);
    const dispatched = [];
    mockService('action', {
        doAction: (action) => dispatched.push(action),
        loadAction: () => ({}),
...
test('restore is a noop while no revision is selected', async () => {
    mockHistory([]);
    let restored = 0;
    onRpc('aypatech_ai.agent', 'prompt_history_restore', () => {
        restored++;
        return false;
    });
    let closed = 0;
...
test('the history action opens the dialog and closes the action window', async () => {
    ...
    const handler = registry.category('actions').get('aypatech_ai_assistant.ai_prompt_history_dialog');
    const result = handler(env, {
        params: { res_model: 'aypatech_ai.agent', res_id: 3, field_name: 'system_prompt' },
    });
