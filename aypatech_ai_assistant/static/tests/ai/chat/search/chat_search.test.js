...
test('buildIndex picks user text and assistant text-blocks only', () => {
    ...
    expect(idx[0].anchorId).toBe('ayp_msg_0_user');
    expect(idx[1].anchorId).toBe('ayp_msg_1_0');
    expect(idx[2].anchorId).toBe('ayp_msg_1_2');
});
...
test('escapeAndHighlight HTML-escapes input then wraps the active match', () => {
    ...
    expect(out.includes('ayp_search_hit_active')).toBe(true);
    expect(out.includes('ayp_search_hit')).toBe(true);
    const active = (out.match(/ayp_search_hit_active/g) || []).length;
...
test('highlightHtml preserves anchor href and skips script/style/mark', () => {
    const html =
        ...
        '<mark class="ayp_search_hit">hello</mark>';
    ...
    const total =
        (out.match(/ayp_search_hit(?!_active)/g) || []).length +
        (out.match(/ayp_search_hit_active/g) || []).length;
...
test('only the activeMatchIdx-th hit gets the active class', () => {
    ...
    const activeCount = (html.match(/ayp_search_hit_active/g) || []).length;
    expect(activeCount).toBe(1);
    const totalHits = (html.match(/ayp_search_hit(?!_active)/g) || []).length;
