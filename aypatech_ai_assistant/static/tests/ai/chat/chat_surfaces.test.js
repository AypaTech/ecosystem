...
function baseMocks({ events = [], record = {} } = {}) {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotWith(events));
...
test('clicking an image rendered by markdown opens it in the file viewer', async () => {
    ...
    const img = queryFirst('img.ayp_md_image');
...
test('pasting into the composer attaches the file exactly once', async () => {
    ...
    const textarea = queryFirst('.ayp_composer textarea');
...
test('a session owned by somebody else is read only', async () => {
    ...
    expect(queryFirst('.ayp_composer_readonly')).not.toBe(null);
    expect(queryFirst('.ayp_composer_row')).toBe(null);
});
...
test('every write action of a read-only session is a no-op', async () => {
    ...
    onRpc('aypatech_ai.session', 'write', () => {
        calls++;
        return true;
    });
...
test('popping out on a small screen opens the full-page action instead', async () => {
    ...
    expect(actions[0][0]).toBe('aypatech_ai_assistant.ai_action_ai_chat');
...
test('the context gauge colours by fill ratio', async () => {
    ...
    expect(chat.contextClass).toBe('ayp_context_green');
    chat.session.state.lastInputTokens = 6000;
    expect(chat.contextPercent).toBe(75);
    expect(chat.contextClass).toBe('ayp_context_amber');
    chat.session.state.lastInputTokens = 7600;
    expect(chat.contextClass).toBe('ayp_context_red');
