...
import { collectSources } from '@aypatech_ai_assistant/ai/chat/artifacts/types/sources_type';
import { SourceList } from '@aypatech_ai_assistant/ai/chat/artifacts/types/sources_tab';
...
test('SourceList caps the display and reveals the rest on +N more', async () => {
    ...
    expect(queryAll('.ayp_source_card').length).toBe(8);
    const more = queryFirst('.ayp_sources_more_btn');
    expect(more.textContent).toInclude('4 more');
    await click(more);
    await animationFrame();
    expect(queryAll('.ayp_source_card').length).toBe(12);
});
...
test('SourceList shows no +N more when within the cap', async () => {
    ...
    expect(queryAll('.ayp_source_card').length).toBe(3);
    expect(queryFirst('.ayp_sources_more_btn')).toBe(null);
});
