    ...
    class Parent extends Component {
        ...
        static template = xml`
            <ToolCard
                ...
                onToggle="this.props.onToggle or (() => {})"
            />
        `;
    }
    Parent.props = {
...
test('renders the tool name and a wrench icon when collapsed and idle', async () => {
    ...
    expect(queryFirst('.ayp_tool_name').textContent).toBe('list_modules');
    expect('.fa-wrench').toHaveCount(1);
    expect('.fa-circle-o-notch').toHaveCount(0);
    expect('.ayp_tool_body').toHaveCount(0);
});
...
test('streaming swaps wrench for spinner and shows running… status', async () => {
    ...
    expect(queryFirst('.ayp_tool_status').textContent.trim()).toBe('running…');
});
...
test('summary inlines whitespace and truncates over 60 chars with an ellipsis', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const summary = queryFirst('.ayp_tool_preview').textContent;
...
test('summary is empty when block has no arguments', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect(queryFirst('.ayp_tool_preview').textContent).toBe('');
});
...
test('expanded shows pretty-printed arguments and parsed JSON-string result', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_body').toHaveCount(1);
    expect('.ayp_tool_section').toHaveCount(2);
    const code = document.querySelectorAll('.ayp_tool_section code');
...
test('expanded with non-JSON string result keeps it verbatim', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect(queryFirst('.ayp_tool_section code').textContent).toBe('plain answer');
});
...
test('expanded with no result shows the running placeholder', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_section .fa-spinner').toHaveCount(1);
    expect(document.querySelectorAll('.ayp_tool_section').length).toBe(2);
});
...
test('applies ayp_tool_write kind class for create/update tool names', async () => {
    const { Parent, props } = makeParent({
        block: { name: 'create_record', arguments: null, callId: 'c1' },
    });
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_write').toHaveCount(1);
});

test('applies ayp_tool_read kind class for search tool names', async () => {
    const { Parent, props } = makeParent({
        block: { name: 'search_read', arguments: null, callId: 'c2' },
    });
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_read').toHaveCount(1);
});

test('applies ayp_tool_nav kind class for open tool names', async () => {
    const { Parent, props } = makeParent({
        block: { name: 'open_record', arguments: null, callId: 'c3' },
    });
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_nav').toHaveCount(1);
});

test('applies ayp_tool_error when result contains an error', async () => {
    const { Parent, props } = makeParent({
        block: {
            name: 'search_read',
            arguments: null,
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_error').toHaveCount(1);
});

test('applies ayp_tool_error when result JSON string contains an error', async () => {
    const { Parent, props } = makeParent({
        block: {
            name: 'search_read',
            arguments: null,
...
test('falls back to default kind for unknown tool name', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_default').toHaveCount(1);
});
