...
import {
    ...
    buildTurnItems,
    toolBlockHasError,
} from '@aypatech_ai_assistant/ai/chat/tools/tool_group';
    ...
    class Parent extends Component {
        ...
        static template = xml`
            <ToolGroup
                ...
                isToolExpanded="this.props.isToolExpanded"
                onToggleTool="this.props.onToggleTool"
            />
        `;
    }
    Parent.props = {
...
test('ToolGroup: header shows the tool count and is collapsed by default', async () => {
    ...
    expect(queryFirst('.ayp_tool_group_label').textContent.trim()).toBe('Used 2 tools');
    expect('.ayp_tool_group_body').toHaveCount(0);
    expect(queryFirst('.ayp_tool_group').dataset.expanded).toBe('0');
});
...
test('ToolGroup: clicking the head expands and reveals an inner card per tool', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    await click('.ayp_tool_group_head');
    await animationFrame();
    expect('.ayp_tool_group_body').toHaveCount(1);
    expect('.ayp_tool_group_body .ayp_tool').toHaveCount(3);
    expect(queryFirst('.ayp_tool_group').dataset.expanded).toBe('1');
    await click('.ayp_tool_group_head');
    await animationFrame();
    expect('.ayp_tool_group_body').toHaveCount(0);
});
...
test('ToolGroup: all-success cluster shows only an ok badge', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tg_ok').toHaveCount(1);
    expect('.ayp_tg_err').toHaveCount(0);
    expect(queryFirst('.ayp_tg_ok').textContent.trim()).toBe('2');
    expect('.ayp_tool_group_has_error').toHaveCount(0);
});
...
test('ToolGroup: a failing tool adds an error badge and the error class', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect(queryFirst('.ayp_tg_ok').textContent.trim()).toBe('1');
    expect(queryFirst('.ayp_tg_err').textContent.trim()).toBe('1');
    expect('.ayp_tool_group_has_error').toHaveCount(1);
});

test('ToolGroup: chips list each tool name', async () => {
    const { Parent, props } = makeGroup([tool('search_records'), tool('read_records')]);
    await mountWithCleanup(Parent, { props });
    expect(queryAllTexts('.ayp_tool_group_chip')).toEqual([
        'search_records',
        'read_records',
    ]);
});
...
test('ToolGroup: chips cap at four names and add an ellipsis overflow chip', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_group_chip').toHaveCount(5);
    expect(queryFirst('.ayp_tool_group_chip_more').textContent.trim()).toBe('…');
});
...
test('ToolGroup: compact mode hides the chips but keeps count and badges', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_tool_group_chips').toHaveCount(0);
    expect(queryFirst('.ayp_tool_group_label').textContent.trim()).toBe('Used 2 tools');
    expect('.ayp_tg_ok').toHaveCount(1);
    expect('.ayp_tg_err').toHaveCount(1);
});
...
test('ToolGroup: inner card toggle bubbles the callId to onToggleTool', async () => {
    ...
    await click('.ayp_tool_group_head');
    await animationFrame();
    const heads = queryAll('.ayp_tool_group_body .ayp_tool_head');
    await click(heads[1]);
