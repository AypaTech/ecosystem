    ...
    class Parent extends Component {
        ...
        static template = xml`
            <ChatComposer
                ...
                onInput="() => {}"
                onSend="() => {}"
                onAttachFiles="this.props.onAttachFiles or (() => {})"
                onRemoveAttachment="this.props.onRemoveAttachment or (() => {})"
                onOpenAttachment="this.props.onOpenAttachment or (() => {})"
            />
        `;
    }
    Parent.props = {
...
test('renders no cards when attachments is empty', async () => {
    ...
    expect('.ayp_att_card').toHaveCount(0);
    expect('.ayp_attach').toHaveCount(1);
});
...
test('renders image thumb for image attachment', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_att_card').toHaveCount(1);
    expect('.ayp_att_card_thumb').toHaveCount(1);
    expect('.ayp_att_card .o_image').toHaveCount(0);
    expect(queryFirst('.ayp_att_card_thumb').getAttribute('alt')).toBe('pic.png');
});
...
test('renders o_image tile with mimetype for pdf attachment', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_att_card').toHaveCount(1);
    const img = queryFirst('.ayp_att_card .o_image');
    expect(img).not.toBe(null);
    expect(img.getAttribute('data-mimetype')).toBe('application/pdf');
    expect('.ayp_att_card_thumb').toHaveCount(0);
});
...
test('renders o_image tile with mimetype for plain text attachment', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect(queryFirst('.ayp_att_card .o_image').getAttribute('data-mimetype')).toBe(
        'text/plain',
    );
});
...
test('removes attachment via the card remove button', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_att_card_remove').toHaveCount(1);
    await click('.ayp_att_card_remove');
...
test('disables attach button when canAttach is false', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const label = queryAll('.ayp_attach')[0];
    expect(label).not.toBe(undefined);
    expect(label.classList.contains('ayp_attach_disabled')).toBe(true);
    const input = queryFirst('.ayp_file_input');
    ...
    class Parent extends Component {
        ...
        static template = xml`
            <ChatComposer
                ...
                canStop="this.props.canStop"
                canAttach="this.props.canAttach"
                attachments="[]"
                agents="this.props.agents"
                activeAgentId="this.props.activeAgentId"
                onInput="this.props.onInput or (() => {})"
                onSend="this.props.onSend or (() => {})"
                onStop="this.props.onStop or (() => {})"
                onAttachFiles="this.props.onAttachFiles or (() => {})"
                onSelectAgent="this.props.onSelectAgent or (() => {})"
            />
        `;
    }
    Parent.props = {
...
test('Enter triggers onSend when canSend=true', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const area = queryFirst('.ayp_composer textarea');
...
test('Enter routes to onStop when canStop=true takes priority over canSend', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    queryFirst('.ayp_composer textarea').dispatchEvent(
...
test('typing / shows slash command menu', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const items = queryAll('.ayp_slash_item');
...
test('clicking an agent item calls onSelectAgent', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    await click(queryAll('.ayp_slash_item')[2]);
...
test('slash menu ArrowDown cycles active entry', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const area = queryFirst('.ayp_composer textarea');
    const before = queryAll('.ayp_slash_item').map((el) => el.className);
    area.dispatchEvent(
        new KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true }),
    );
    const after = queryAll('.ayp_slash_item').map((el) => el.className);
...
test('typing /c lists /compact before /clear', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const items = queryAll('.ayp_slash_name');
    expect(items[0].textContent).toMatch(/\/compact/);
...
test('focusToken bump never steals focus from another text input', async () => {
    ...
    class Parent extends Component {
        ...
        static template = xml`
            <div>
                <input class="ayp_other_input" type="text"/>
                <ChatComposer
                    value="''"
                    placeholder="'type'"
                    canSend="false"
        ...
        `;
        setup() {
            this.state = proxy({ token: 0 });
            parent = this;
        }
    }
    await mountWithCleanup(Parent, { props: {} });
    const other = queryFirst('.ayp_other_input');
    const composer = queryFirst('.ayp_composer textarea');
    other.focus();
