    ...
    class Parent extends Component {
        ...
        static template = xml`
            <ChatSidebar
                ...
                onNew="this.props.onNew or (() => {})"
                onSelect="this.props.onSelect or (() => {})"
                onRename="this.props.onRename or (() => {})"
                onDelete="this.props.onDelete or (() => {})"
            />
        `;
    }
    Parent.props = {
...
function bucketsFromDom() {
    const buckets = {};
    let current = null;
    for (const node of queryAll('.ayp_sidebar_group_label, .ayp_sidebar_item')) {
        if (node.classList.contains('ayp_sidebar_group_label')) {
            current = node.textContent.trim();
            buckets[current] = [];
        } else if (current) {
            const name = node.querySelector('.ayp_sidebar_name');
...
test('shows empty-state when no sessions', async () => {
    ...
    expect('.ayp_sidebar_item').toHaveCount(0);
    expect('.ayp_sidebar_search').toHaveCount(0);
    expect(queryFirst('.ayp_sidebar_list').textContent).toMatch(/No chats yet/);
});
...
test('groups sessions into Today / Yesterday / Previous 7 days / Previous 30 days / Older', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const labels = queryAll('.ayp_sidebar_group_label').map((el) =>
        el.textContent.trim(),
    );
...
test('search filters by name (case-insensitive, substring)', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    expect('.ayp_sidebar_item').toHaveCount(3);
    await contains('.ayp_sidebar_search_input').edit('SALES', { confirm: false });
    expect('.ayp_sidebar_item').toHaveCount(2);
    expect(queryAll('.ayp_sidebar_name').map((el) => el.textContent)).toEqual([
        'Sales pipeline',
        'sales follow-up',
    ]);
});
...
test('search empty-state when no match', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    await contains('.ayp_sidebar_search_input').edit('zzz', { confirm: false });
    expect('.ayp_sidebar_item').toHaveCount(0);
    expect(queryFirst('.ayp_sidebar_list').textContent).toMatch(/No chats match/);
    expect(queryFirst('.ayp_sidebar_list strong').textContent).toBe('zzz');
});
...
test('clear button resets the query', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    await contains('.ayp_sidebar_search_input').edit('foo', { confirm: false });
    expect('.ayp_sidebar_item').toHaveCount(1);
    await contains('.ayp_sidebar_search_clear').click();
    expect('.ayp_sidebar_item').toHaveCount(2);
});
...
test('marks active session and emits onSelect on click', async () => {
    ...
    await mountWithCleanup(Parent, { props });
    const items = queryAll('.ayp_sidebar_item');
...
test('running and waiting states get visual indicators', async () => {
    ...
    expect('.ayp_state_done .fa').toHaveCount(0);
    expect(queryAll('.ayp_sidebar_item.ayp_running').length).toBe(2);
});
