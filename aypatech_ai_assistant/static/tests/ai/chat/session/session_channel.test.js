...
import {
    sessionChannel,
    useSessionChannel,
} from '@aypatech_ai_assistant/ai/chat/session/session_channel';
import { isChatOpen, useOpenSession } from '@aypatech_ai_assistant/ai/chat/session/open_sessions';
...
function makeFollower() {
    class Follower extends Component {
        ...
        setup() {
            this.state = proxy({ sessionId: this.props.sessionId ?? null });
...
test('sessionChannel names the session record channel', () => {
    expect(sessionChannel(42)).toBe('aypatech_ai_assistant.ai_session_42');
});
...
test('a reader follows the chat but never answers its client actions', async () => {
    ...
    class Reader extends Component {
        ...
        static template = xml`<div/>`;
        setup() {
            this.state = proxy({ sessionId: 31, readonly: true });
            useSessionChannel(() => this.state.sessionId);
