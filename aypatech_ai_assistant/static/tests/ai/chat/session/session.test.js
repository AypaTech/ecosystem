...
function makeBusMock() {
    ...
    return {
        emit(payload) {
            const cb = handlers.get('aypatech_ai_assistant.ai_event');
            if (cb) {
                cb(payload);
            }
        },
...
function makeHarness(options = {}) {
    ...
    class Harness extends Component {
        props = useProps({});
        static template = xml`<div class="ayp_harness"/>`;
        setup() {
...
test('load reads session and applies record to state', async () => {
    onRpc('aypatech_ai.session', 'read', ({ args }) => {
        expect(args[0]).toEqual([7]);
        return [SESSION_RECORD];
    });
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(SESSION_RECORD));
    makeBusMock();
...
test('load never bumps focusToken', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(SESSION_RECORD));
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('load is atomic: previous events stay visible until new snapshot arrives', async () => {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', ({ args }) => {
        if (args[0] === 7) {
...
test('optimistic user message keeps its render identity across the persisted swap', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.session', 'send_message', () => ({
        ...SNAPSHOT_RUNNING,
        events: [
...
test('bus echo replaces the optimistic twin in place instead of appending', async () => {
    ...
    onRpc('aypatech_ai.session', 'send_message', () => sendGate);
    const bus = makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('canSend is false while load is in flight', async () => {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(SECOND_RECORD));
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness, 7);
...
test('loadAgents fetches agents via search_read with suggestions field', async () => {
    let captured = null;
    onRpc('aypatech_ai.agent', 'search_read', ({ kwargs }) => {
        captured = kwargs;
        return AGENTS;
    });
    makeBusMock();
...
test('onSend posts to start for the first turn and flips status to running', async () => {
    const calls = [];
    onRpc('aypatech_ai.session', 'read', () => [
        { ...SESSION_RECORD, events: [], iteration_count: 0 },
    ]);
    onRpc('aypatech_ai.session', 'start', ({ args, kwargs }) => {
...
test('onSend routes subsequent turns through send_message', async () => {
    let sent = false;
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.session', 'send_message', ({ args }) => {
        sent = true;
...
test('onSend queues through enqueue_message while a turn is running', async () => {
    ...
    onRpc('aypatech_ai.session', 'read', () => [running]);
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(running));
    const calls = [];
    onRpc('aypatech_ai.session', 'enqueue_message', ({ args }) => {
        calls.push(args);
...
test('rejected queue attempt re-dispatches through the regular send path', async () => {
    ...
    onRpc('aypatech_ai.session', 'read', () => [running]);
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(running));
    onRpc('aypatech_ai.session', 'enqueue_message', () =>
...
test('queue rejection re-send preserves a draft typed meanwhile', async () => {
    ...
    onRpc('aypatech_ai.session', 'enqueue_message', () => enqueueGate);
    onRpc('aypatech_ai.session', 'send_message', () => ({ ...SNAPSHOT_RUNNING }));
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('onSend while waiting on a question dispatches to answer endpoint', async () => {
    ...
    let answered = false;
    onRpc('aypatech_ai.session', 'answer', ({ args }) => {
        answered = true;
...
test('slash command /compact calls compact and skips send_message', async () => {
    ...
    onRpc('aypatech_ai.session', 'compact', ({ args }) => {
        compacted = true;
...
test('onSetAgent writes agent_id and updates state', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.agent', 'search_read', () => AGENTS);
    let written = null;
    onRpc('aypatech_ai.session', 'write', ({ args }) => {
        written = args;
        return true;
    });
    makeBusMock();
...
test('onSend surfaces backend errors into state.error and status', async () => {
    ...
    onRpc('aypatech_ai.session', 'start', () => {
...
test('onStop calls action_stop and applies snapshot', async () => {
    onRpc('aypatech_ai.session', 'read', () => [{ ...SESSION_RECORD, state: 'running' }]);
    let stopped = false;
    onRpc('aypatech_ai.session', 'action_stop', () => {
        stopped = true;
...
test('onRegenerate calls regenerate_last_turn and applies snapshot', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    let called = false;
    onRpc('aypatech_ai.session', 'regenerate_last_turn', ({ args }) => {
        called = true;
        expect(args).toEqual([7]);
        return { ...SNAPSHOT_RUNNING, state: 'done' };
    });
...
test('onRegenerate is blocked while running or waiting', async () => {
    onRpc('aypatech_ai.session', 'read', () => [{ ...SESSION_RECORD, state: 'running' }]);
    let attempts = 0;
    onRpc('aypatech_ai.session', 'regenerate_last_turn', () => {
        attempts++;
        return SNAPSHOT_RUNNING;
    });
    makeBusMock();
...
test('canRegenerate is false when log has no user_message', async () => {
    onRpc('aypatech_ai.session', 'read', () => [{ ...SESSION_RECORD, events: [] }]);
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('loadAgents swallows errors and resets agents to empty', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.agent', 'search_read', () => {
...
test('setApprovalMode calls set_approval_mode on the session', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    let captured = null;
    onRpc('aypatech_ai.session', 'set_approval_mode', ({ args }) => {
        captured = args;
...
test('runUnpin warns when no view context is pinned', async () => {
    onRpc('aypatech_ai.session', 'read', () => [{ ...SESSION_RECORD, view_context: null }]);
    const notifications = [];
    mockService('notification', {
        add: (msg, opts) => notifications.push({ msg: String(msg), type: opts?.type }),
    });
...
test('runUnpin calls unpin_view_context when context is set', async () => {
    ...
    let called = false;
    onRpc('aypatech_ai.session', 'unpin_view_context', () => {
        called = true;
...
test('approveTool fires approve_tool RPC when awaiting approval', async () => {
    ...
    let approved = false;
    onRpc('aypatech_ai.session', 'approve_tool', () => {
        approved = true;
        return { ...SNAPSHOT_RUNNING, state: 'running' };
    });
    makeBusMock();
...
test('approveForSession fires approve_for_session RPC when awaiting approval', async () => {
    ...
    let called = false;
    onRpc('aypatech_ai.session', 'approve_for_session', () => {
        called = true;
        return SNAPSHOT_RUNNING;
    });
    makeBusMock();
...
test('rejectTool fires reject_tool with reason kwarg', async () => {
    ...
    onRpc('aypatech_ai.session', 'reject_tool', ({ kwargs }) => {
...
test('renderedTurns groups log entries into user + assistant turns', async () => {
    ...
    onRpc('aypatech_ai.session', 'read', () => [record]);
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(record));
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('maybeAutoCompact fires /compact silently when ratio above auto threshold', async () => {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(record));
    let compacted = false;
    onRpc('aypatech_ai.session', 'compact', () => {
        compacted = true;
...
test('onAttachFiles uploads base64 payloads and appends to pendingAttachments', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    let payloads = null;
    onRpc('aypatech_ai.session', 'upload_attachments', ({ args }) => {
        payloads = args[1];
...
test('onAttachFiles is a noop when files array is empty', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    let attempts = 0;
    onRpc('aypatech_ai.session', 'upload_attachments', () => {
        attempts++;
        return [];
    });
    makeBusMock();
...
test('onRemoveAttachment filters the attachment and calls discard RPC', async () => {
    ...
    onRpc('aypatech_ai.session', 'discard_attachments', ({ args }) => {
        discardArgs = args;
        return true;
    });
    makeBusMock();
...
test('answerWithOption is a noop during running status', async () => {
    onRpc('aypatech_ai.session', 'read', () => [{ ...SESSION_RECORD, state: 'running' }]);
    let attempts = 0;
    onRpc('aypatech_ai.session', 'answer', () => {
        attempts++;
        return SNAPSHOT_RUNNING;
    });
    makeBusMock();
...
test('onSend emits optimistic answer entry when waiting on a question', async () => {
    ...
    onRpc('aypatech_ai.session', 'answer', () => SNAPSHOT_RUNNING);
    makeBusMock();
    const harness = makeHarness();
    const session = await mountAndLoad(harness);
...
test('setApprovalMode swallows errors via notification', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    onRpc('aypatech_ai.session', 'set_approval_mode', () => {
        throw new Error('boom');
    });
    const notifications = [];
...
test('/clear calls clear immediately without a confirmation dialog', async () => {
    onRpc('aypatech_ai.session', 'read', () => [SESSION_RECORD]);
    let cleared = false;
    onRpc('aypatech_ai.session', 'clear', () => {
        cleared = true;
...
test('loadMoreEvents prepends older window and updates oldestSequence', async () => {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(record));
    let fetchArgs = null;
    onRpc('aypatech_ai.session', 'fetch_events', ({ args, kwargs }) => {
...
test('loadMoreEvents is a noop when hasMoreOlder is false', async () => {
    ...
    onRpc('aypatech_ai.session', 'get_snapshot', () => snapshotFor(SESSION_RECORD));
    let calls = 0;
    onRpc('aypatech_ai.session', 'fetch_events', () => {
        calls++;
