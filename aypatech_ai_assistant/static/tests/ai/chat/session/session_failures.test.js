...
test('cancelQueued removes the entry and adopts the returned queue', async () => {
    ...
    let cancelArgs = null;
    onRpc('aypatech_ai.session', 'cancel_queued', ({ args }) => {
        cancelArgs = args;
...
test('a failed cancel puts the queued message back at its own position', async () => {
    ...
    onRpc('aypatech_ai.session', 'cancel_queued', () => {
...
test('a nearly full context window warns once before the next send', async () => {
    ...
    onRpc('aypatech_ai.session', 'send_message', () => snapshotFor(record));
    let compacted = 0;
...
test('a ui_action without a real action payload is ignored', async () => {
    ...
    await mountSession();
    bus.emit({ session_id: 7, type: 'ui_action', payload: { action: 'aypatech_ai_assistant.ai_foo' } });
