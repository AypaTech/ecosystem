...
test('the overlay shows the current step and fills the progress bar', async () => {
    ...
    expect('.ayp_block_progress_overlay').toHaveCount(1);
    expect(queryText('.ayp_block_progress_card')).toInclude('Batch');
    expect(queryText('.ayp_block_progress_card')).toInclude('2');
    expect(queryText('.ayp_block_progress_card')).toInclude('4');
...
test('no estimate is shown while the progress is still at zero', async () => {
    ...
    expect(queryText('.ayp_block_progress_card')).not.toInclude('Estimated time left');
});
...
test('the estimate is shown in minutes once it exceeds one', async () => {
    ...
    await animationFrame();
    expect(queryText('.ayp_block_progress_card')).toInclude('Estimated time left');
    expect(queryText('.ayp_block_progress_card')).toInclude('1.50');
    expect(queryText('.ayp_block_progress_card')).toInclude('minutes');
});
...
test('the estimate falls back to seconds below one minute', async () => {
    ...
    await advanceTime(60000);
    await animationFrame();
    expect(queryText('.ayp_block_progress_card')).toInclude('20');
    expect(queryText('.ayp_block_progress_card')).toInclude('seconds');
    expect(queryText('.ayp_block_progress_card')).not.toInclude('minutes');
});
