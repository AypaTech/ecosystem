...
import { probeModuleAvailable } from '@aypatech_ai_assistant/webutils/views/module_availability';
...
test('an existing module resolves true and queries ir.module.module by name', async () => {
    const orm = makeOrm(1);
    expect(await probeModuleAvailable(orm, 'aypatech_probe_present')).toBe(true);
    expect(orm.calls).toHaveLength(1);
    expect(orm.calls[0][0]).toBe('ir.module.module');
    expect(orm.calls[0][1]).toEqual([['name', '=', 'aypatech_probe_present']]);
});

test('a missing module resolves false', async () => {
    const orm = makeOrm(0);
    expect(await probeModuleAvailable(orm, 'aypatech_probe_absent')).toBe(false);
});

test('the result is cached so the module is probed only once', async () => {
    const orm = makeOrm(1);
    await probeModuleAvailable(orm, 'aypatech_probe_cached');
    await probeModuleAvailable(orm, 'aypatech_probe_cached');
    const other = makeOrm(0);
    expect(await probeModuleAvailable(other, 'aypatech_probe_cached')).toBe(true);
    expect(orm.calls).toHaveLength(1);
...
test('a failing probe resolves false instead of rejecting', async () => {
    ...
    expect(await probeModuleAvailable(orm, 'aypatech_probe_error')).toBe(false);
});
