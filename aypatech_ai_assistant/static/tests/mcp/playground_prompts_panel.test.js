...
import { STORAGE_KEY } from '@aypatech_ai_assistant/mcp/playground/mcp_client';
import { PromptsPanel } from '@aypatech_ai_assistant/mcp/playground/prompts_panel';

describe.current.tags('aypatech_mcp');

defineMailModels();

const LAST_PROMPT_KEY = 'aypatech_ai_assistant.mcp_playground.last_prompt';
const ARGS_PREFIX = 'aypatech_ai_assistant.mcp_playground.prompt_args::';
...
function mockPanelServices({ prompts = [GREETING, SUMMARY], loadError = null } = {}) {
    ...
    mockService('orm', {
        call(model, method) {
            if (model === 'aypatech_mcp.prompt' && method === 'get_playground_prompts') {
...
function argInputs() {
    return queryAll('.o_aypatech_mcp_detail input[type="text"]');
}
...
test('renders the prompt catalog and selects the first prompt', async () => {
    ...
    expect('.o_aypatech_mcp_tool').toHaveCount(2);
    expect(queryAllTexts('.o_aypatech_mcp_tool .fw-bold')).toEqual(['greeting', 'summary']);
    expect('.o_aypatech_mcp_tool.active .fw-bold').toHaveText('greeting');
    expect('.o_aypatech_mcp_detail h4').toHaveText('greeting');
    expect('.o_aypatech_mcp_detail').toHaveText(/Say hello to a partner/);
});
...
test('restores the last used prompt from localStorage', async () => {
    ...
    mockPanelServices();
    await mountWithCleanup(PromptsPanel, { props: {} });
    expect('.o_aypatech_mcp_tool.active .fw-bold').toHaveText('summary');
    expect('.o_aypatech_mcp_detail').toHaveText(/This prompt takes no arguments/);
});
...
test('notifies and renders an empty catalog when loading fails', async () => {
    ...
    expect('.o_aypatech_mcp_tool').toHaveCount(0);
    expect('.o_aypatech_mcp_list').toHaveText(/No matching prompts/);
    expect('.o_aypatech_mcp_detail').toHaveText(/Select a prompt from the list on the left/);
});

test('filters the prompt list on name, title and description', async () => {
    mockPanelServices();
    await mountWithCleanup(PromptsPanel, { props: {} });
    await typeInto('.o_aypatech_mcp_list input[type="search"]', 'GREET');
    expect(queryAllTexts('.o_aypatech_mcp_tool .fw-bold')).toEqual(['greeting']);
    await typeInto('.o_aypatech_mcp_list input[type="search"]', '  Record summary ');
    expect(queryAllTexts('.o_aypatech_mcp_tool .fw-bold')).toEqual(['summary']);
    await typeInto('.o_aypatech_mcp_list input[type="search"]', 'summarise');
    expect(queryAllTexts('.o_aypatech_mcp_tool .fw-bold')).toEqual(['summary']);
    await typeInto('.o_aypatech_mcp_list input[type="search"]', 'nothing-here');
    expect('.o_aypatech_mcp_tool').toHaveCount(0);
    expect('.o_aypatech_mcp_list').toHaveText(/No matching prompts/);
});
...
test('selecting a prompt persists it and swaps the detail pane', async () => {
    mockPanelServices();
    await mountWithCleanup(PromptsPanel, { props: {} });
    await click(queryAll('.o_aypatech_mcp_tool')[1]);
    await animationFrame();
    expect('.o_aypatech_mcp_detail h4').toHaveText('summary');
...
test('reset args clears every argument of the active prompt', async () => {
    ...
    await mountWithCleanup(PromptsPanel, { props: {} });
    await typeInto(argInputs()[0], 'Alice');
    await click('.o_aypatech_mcp_detail .btn-outline-secondary');
    await animationFrame();
    expect(argInputs()[0].value).toBe('');
    expect(JSON.parse(localStorage.getItem(ARGS_PREFIX + 'greeting'))).toEqual({
        partner: '',
...
test('running a prompt sends pruned arguments and renders the messages', async () => {
    ...
    await mountWithCleanup(PromptsPanel, { props: {} });
    await typeInto(argInputs()[0], 'Alice');
    await click('.o_aypatech_mcp_detail .btn-primary');
    await animationFrame();
    expect(params).toEqual({ name: 'greeting', arguments: { partner: 'Alice' } });
    expect(
        queryAll('.o_aypatech_mcp_detail .badge').map((b) => b.textContent.trim()),
    ).toEqual(['user', 'assistant']);
    expect(queryAll('.o_aypatech_mcp_detail pre')[0]).toHaveText('Hello Alice');
    expect(queryAll('.o_aypatech_mcp_detail pre')[1].textContent).toInclude(
        '"uri": "odoo://partner/1"',
    );
    expect('.o_aypatech_mcp_detail .fw-bold.text-success').toHaveText('HTTP 200');
    expect(queryOne('.o_aypatech_mcp_detail details pre').textContent).toInclude(
        '"description": "Rendered greeting"',
    );
});
...
test('selecting another prompt clears the previous response', async () => {
    ...
    await click('.o_aypatech_mcp_detail .btn-primary');
    await animationFrame();
    expect('.o_aypatech_mcp_detail details').toHaveCount(1);
    await click(queryAll('.o_aypatech_mcp_tool')[1]);
    await animationFrame();
    expect('.o_aypatech_mcp_detail details').toHaveCount(0);
});
...
test('a JSON-RPC error envelope is surfaced as an alert', async () => {
    ...
    await click('.o_aypatech_mcp_detail .btn-primary');
    await animationFrame();
    expect('.o_aypatech_mcp_detail .alert-danger').toHaveText(
        'JSON-RPC error -32602: Missing required argument',
    );
    expect('.o_aypatech_mcp_detail .fw-bold.text-warning').toHaveText('HTTP 400');
...
test('a non-JSON server failure falls back to the raw body', async () => {
    ...
    await click('.o_aypatech_mcp_detail .btn-primary');
    await animationFrame();
    expect('.o_aypatech_mcp_detail .fw-bold.text-danger').toHaveText('HTTP 502');
    expect(queryOne('.o_aypatech_mcp_detail details pre').textContent).toBe(
        '502 Bad Gateway',
    );
    expect('.o_aypatech_mcp_detail').toHaveText(/No messages returned/);
});
...
test('ctrl+enter runs the selected prompt', async () => {
    ...
    expect('.o_aypatech_mcp_detail pre').toHaveCount(2);
});

test('the run button is disabled and warns while no key is set', async () => {
    mockPanelServices();
    await mountWithCleanup(PromptsPanel, { props: {} });
    expect(queryOne('.o_aypatech_mcp_detail .btn-primary').disabled).toBe(true);
    expect('.o_aypatech_mcp_detail').toHaveText(/Pick or generate an MCP key first/);
});
...
test('copy curl writes an authenticated prompts/get command', async () => {
    ...
    await mountWithCleanup(PromptsPanel, { props: {} });
    await typeInto(argInputs()[0], 'Alice');
    await click(queryAll('.o_aypatech_mcp_detail .btn-secondary')[0]);
    await animationFrame();
...
test('copy JSON-RPC writes the indented payload without the key', async () => {
    mockPanelServices();
    await mountWithCleanup(PromptsPanel, { props: {} });
    await click(queryAll('.o_aypatech_mcp_detail .btn-secondary')[1]);
    await animationFrame();
