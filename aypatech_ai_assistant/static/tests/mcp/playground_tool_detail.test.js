...
import { ToolDetail } from '@aypatech_ai_assistant/mcp/playground/tool_detail';
...
test('renders the tool header and seeds the form from the schema', async () => {
    ...
    expect('.o_aypatech_mcp_detail .badge.text-bg-info').toHaveText('read');
    expect(queryAll('.o_aypatech_mcp_detail .badge.text-bg-light')[0]).toHaveText('DB');
    expect('.o_aypatech_mcp_detail').toHaveText(/Search partners/);
    expect(queryOne('.o_aypatech_mcp_detail input[type="text"]').value).toBe('');
    expect(queryOne('.o_aypatech_mcp_detail textarea').value).toBe('[]');
    expect(queryOne('.o_aypatech_mcp_detail input[type="number"]').value).toBe('80');
});
...
test('the schema tab shows the indented input schema', async () => {
    await mountDetail();
    await click(queryAll('.o_aypatech_mcp_detail .nav-link')[1]);
    await animationFrame();
    expect(JSON.parse(queryOne('.o_aypatech_mcp_detail pre').textContent)).toEqual(
        TOOL.inputSchema,
    );
    await click(queryAll('.o_aypatech_mcp_detail .nav-link')[0]);
    await animationFrame();
    expect('.o_aypatech_mcp_detail pre').toHaveCount(0);
    expect('.o_aypatech_mcp_detail textarea').toHaveCount(1);
});
...
test('running emits the pruned arguments of the edited form', async () => {
    await mountDetail();
    await click('.o_aypatech_mcp_detail input[type="text"]');
...
test('a 4xx response is shown as a warning status', async () => {
    ...
    expect('.o_aypatech_mcp_detail .fw-bold.text-warning').toHaveText('HTTP 404');
});
...
test('a 5xx response is shown as a danger status', async () => {
    ...
    expect('.o_aypatech_mcp_detail .fw-bold.text-danger').toHaveText('HTTP 500');
    expect(queryOne('.o_aypatech_mcp_detail details pre').textContent).toBe('boom');
});
