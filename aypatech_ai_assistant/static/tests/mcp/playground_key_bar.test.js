...
import { KeyBar } from '@aypatech_ai_assistant/mcp/playground/key_bar';
import { MCPClient, STORAGE_KEY } from '@aypatech_ai_assistant/mcp/playground/mcp_client';
...
test('shows no key and hides the clear button while none is set', async () => {
    ...
    expect('.o_aypatech_mcp_keybar .btn-outline-danger').toHaveCount(0);
    expect('.o_aypatech_mcp_keybar input[type="password"]').toHaveCount(0);
});

test('pasting a key stores it for the tab and notifies the parent', async () => {
    const client = await mountKeyBar();
    await click('.o_aypatech_mcp_keybar .btn-secondary');
    await animationFrame();
    await click('.o_aypatech_mcp_keybar input[type="password"]');
    await edit('  pasted-key  ');
    await click(queryAll('.o_aypatech_mcp_keybar .btn-primary')[1]);
    await animationFrame();
...
test('confirming an empty key warns and keeps the key unset', async () => {
    ...
    expect('.o_aypatech_mcp_keybar input[type="password"]').toHaveCount(1);
});
...
test('cancelling the paste row discards the typed value', async () => {
    ...
    await click('.o_aypatech_mcp_keybar .btn-link');
    await animationFrame();
    expect('.o_aypatech_mcp_keybar input[type="password"]').toHaveCount(0);
    await click('.o_aypatech_mcp_keybar .btn-secondary');
    await animationFrame();
    expect(queryAll('.o_aypatech_mcp_keybar input[type="password"]')[0].value).toBe('');
...
test('generating a key loads the plaintext and reports the prefix', async () => {
    mockKeyOrm();
    const client = await mountKeyBar();
    await click(queryAll('.o_aypatech_mcp_keybar .btn-primary')[0]);
    await animationFrame();
...
test('the label and scope inputs drive the generate call', async () => {
    mockKeyOrm();
    await mountKeyBar();
    await click('.o_aypatech_mcp_keybar input[type="text"]');
    await edit('CI key');
    await select('read', { target: '.o_aypatech_mcp_keybar select' });
    await click(queryAll('.o_aypatech_mcp_keybar .btn-primary')[0]);
    await animationFrame();
...
test('clearing removes the key from the tab storage', async () => {
    ...
    const client = await mountKeyBar();
    expect('.o_aypatech_mcp_keybar .badge').toHaveText(/live-key/);
    await click('.o_aypatech_mcp_keybar .btn-outline-danger');
    await animationFrame();
    expect(client.key).toBe('');
    expect(sessionStorage.getItem(STORAGE_KEY)).toBe(null);
    expect(keyChanges).toBe(1);
