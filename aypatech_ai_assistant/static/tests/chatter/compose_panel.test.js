...
import { sessionChannel } from '@aypatech_ai_assistant/ai/chat/session/session_channel';

import { ComposePanel } from '@aypatech_ai_assistant/chatter/composer/compose_panel';

describe.current.tags('aypatech_ai_chatter');
defineMailModels();
...
function mockBus() {
    ...
    mockService('bus_service', {
        subscribe(type, handler) {
            if (type === 'aypatech_ai_assistant.ai_event') {
...
function mockServer() {
    ...
    onRpc('aypatech_ai.skill', 'fetch_skills', () => SKILLS);
    onRpc('aypatech_ai.session', 'open_for_composer', ({ kwargs }) => {
        if (calls.openFails) {
    ...
    onRpc('aypatech_ai.session', 'discard_unused_composer', () => {
        calls.discarded = true;
        return true;
    });
    onRpc('aypatech_ai.session', 'update_compose_context', ({ args }) => {
    ...
    onRpc('aypatech_ai.session', 'detach_from_composer', () => {
        calls.detached = true;
        return true;
    });
    onRpc('aypatech_ai.skill', 'save_composer_prompt', ({ args }) => {
...
class PanelHolder extends Component {
    ...
    static template = xml`<ComposePanel t-if="this.state.shown" adapter="this.props.adapter"/>`;

    setup() {
        this.state = proxy({ shown: true });
    }
}
...
test('the chat window takes the session over, and the panel steps aside', async () => {
    ...
    const calls = mockServer();
    const opened = [];
    mockService('aypatech_ai_assistant.ai_chat_window', { open: (id) => opened.push(id) });
