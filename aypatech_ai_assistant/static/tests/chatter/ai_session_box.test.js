...
import { AISessionBox } from '@aypatech_ai_assistant/chatter/chatter/ai_session_box';
import '@aypatech_ai_assistant/chatter/chatter/chatter_patch';
...
test('clicking a foreign session opens the session form instead', async () => {
    ...
    expect(actions[0].res_model).toBe('aypatech_ai.session');
...
test('a session push only refetches when it targets a listed session', async () => {
    ...
    expect(types).toInclude('aypatech_ai_assistant.ai_session_state');
    expect(types).toInclude('aypatech_ai_assistant.ai_event');
    const handler = handlers.get('aypatech_ai_assistant.ai_session_state');
