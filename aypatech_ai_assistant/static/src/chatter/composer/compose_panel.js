import { Component, onMounted, onWillStart, onWillUnmount, proxy, signal, t, useProps } from '@odoo/owl';
...
import { rememberInsert } from '@aypatech_ai_assistant/chatter/composer/chat_insert';
import { wordDiff } from '@aypatech_ai_assistant/chatter/composer/word_diff';
...
export class ComposePanel extends Component {
    static template = 'aypatech_ai_assistant.chatter_ComposePanel';
    props = useProps({
        adapter: t.object(),
    ...
    setup() {
        ...
        useLayoutEffect(
            (open) => {
                if (open) {
                    this.createLabelRef()?.focus();
                }
            },
            () => [this.state.createOpen],
        );
        this.busHandler = (event) => this.onSessionEvent(event);
        this.bus.subscribe('aypatech_ai_assistant.ai_event', this.busHandler);
        ...
        onWillUnmount(() => {
            ...
            this.bus.unsubscribe('aypatech_ai_assistant.ai_event', this.busHandler);
    ...
    discardUnusedSession() {
        ...
        this.orm.silent
            .call('aypatech_ai.session', 'discard_unused_composer', [[this.state.sessionId]])
            .catch(() => {});
    }
    /**
     ...
     * @param {object} event the payload pushed on the aypatech_ai bus
     */
    ...
    async run(label, prompt) {
        ...
        try {
            ...
            await this.orm.call('aypatech_ai.session', 'update_compose_context', [
                [this.state.sessionId],
                this.props.adapter.getDraft(),
                this.isRewrite ? this.target : '',
            ]);
            await this.orm.call('aypatech_ai.session', 'send_message', [
                [this.state.sessionId],
                instruction,
            ]);
        } catch (error) {
    ...
    async onCancel() {
        ...
        if (this.state.sessionId) {
            await this.orm.silent
                .call('aypatech_ai.session', 'action_stop', [[this.state.sessionId]])
                .catch(() => {});
        }
    }
    onRetry() {
    ...
    async onOpenInChat() {
        ...
        await this.orm.call('aypatech_ai.session', 'detach_from_composer', [[sessionId]]);
