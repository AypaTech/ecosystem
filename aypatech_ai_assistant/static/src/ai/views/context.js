...
export function captureViewContext(env, payload) {
    ...
    for (const sessionId of sessionIds) {
        caller
            .call('aypatech_ai.session', 'set_view_context', [sessionId, payload])
            .catch(() => {});
    }
}
...
export async function seedSessionContext(env, sessionId, payload = null) {
    ...
    const caller = orm.silent || orm;
    try {
        await caller.call('aypatech_ai.session', 'set_view_context', [sessionId, ctx]);
        return true;
    } catch {
        return false;
    }
...
function makeDispatch(controller, build) {
    ...
    return () => {
        try {
            const chatWindow = controller.env.services['aypatech_ai_assistant.ai_chat_window'];
            if (!chatWindow) {
                return;
            }
            const sessionIds = chatWindow.sessionIds || [];
