import { isChatOpen } from '@aypatech_ai_assistant/ai/chat/session/open_sessions';
...
/**
 * Build a `aypatech_ai_assistant.ai_event` bus listener that executes client-tool actions.
 *
...
export function makeClientToolListener({ orm, contains, execute, defer }) {
    ...
    async function dispatch(sessionId, callId, name, rawArgs) {
        ...
        if (args === null) {
            await orm
                .call('aypatech_ai.session', 'reject_client_action', [sessionId, callId], {
        ...
        try {
            ...
            await orm.call('aypatech_ai.session', 'submit_client_result', [
                sessionId,
                callId,
                result,
            ]);
    ...
    function dispatchOncePerBrowser(sessionId, callId, name, rawArgs) {
        ...
        if (navigator.locks?.request) {
            navigator.locks.request(
                `aypatech_ai_assistant.ai_client_action.${callId}`,
                { ifAvailable: true },
                async (lock) => {
                    if (!lock) {
                        return;
