import { markup, onWillUnmount, proxy, useEnv } from '@odoo/owl';
...
import { fileToBase64 } from '@aypatech_ai_assistant/ai/core/attachment/file_helpers';
import { renderMarkdown as renderMarkdownToHtml } from '@aypatech_ai_assistant/ai/core/markdown/markdown';
import { formatError } from '@aypatech_ai_assistant/ai/chat/utils';

import { buildRenderedTurns } from '@aypatech_ai_assistant/ai/chat/session/turns';
import { useOpenSession } from '@aypatech_ai_assistant/ai/chat/session/open_sessions';
import { useSessionChannel } from '@aypatech_ai_assistant/ai/chat/session/session_channel';
...
export function useAiSession(options = {}) {
    ...
    const sessionNotification = useService('aypatech_ai_assistant.ai_session_notification');
    const env = useEnv();
    ...
    bus.subscribe('aypatech_ai_assistant.ai_event', busHandler);
    ...
    function maybePopoutBeforeAction() {
        ...
        const chatWindow = env.services?.['aypatech_ai_assistant.ai_chat_window'];
    ...
    async function load(sessionId) {
        ...
        try {
            ...
            if (record) {
                try {
                    snapshot = await orm.call('aypatech_ai.session', 'get_snapshot', [
                        sessionId,
                    ]);
                } catch {
                    snapshot = null;
    ...
    async function cancelQueued(index) {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'cancel_queued', [
                state.sessionId,
                index,
            ]);
            applySnapshot(snapshot);
    ...
    async function onStop() {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'action_stop', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
        } catch (error) {
    ...
    async function onRegenerate() {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'regenerate_last_turn', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
        } catch (error) {
    ...
    async function onAttachFiles(files) {
        ...
        try {
            ...
            const descriptors = await orm.call('aypatech_ai.session', 'upload_attachments', [
                state.sessionId,
                payloads,
            ]);
    ...
    async function runUnpin() {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'unpin_view_context', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
        } catch (error) {
    ...
    async function setApprovalMode(mode) {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'set_approval_mode', [
                state.sessionId,
                mode || false,
            ]);
            applySnapshot(snapshot);
    ...
    async function runClear() {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'clear', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
    ...
    async function runCompact({ silent: _silent }) {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'compact', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
        } catch (error) {
    ...
    async function runStopCompact() {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'stop_compact', [
                state.sessionId,
            ]);
            applySnapshot(snapshot);
        } catch (error) {
    ...
    async function runUndoToEvent(eventId) {
        ...
        try {
            const snapshot = await orm.call('aypatech_ai.session', 'undo_to_event', [
                state.sessionId,
                eventId,
            ]);
            applySnapshot(snapshot);
    ...
    async function runForkAtEvent(eventId) {
        ...
        try {
            const newId = await orm.call('aypatech_ai.session', 'fork_at_event', [
                state.sessionId,
                eventId,
            ]);
    ...
    async function onHandover(userId) {
        ...
        try {
            await orm.call('aypatech_ai.session', 'action_handover', [
                state.sessionId,
                userId,
            ]);
    ...
    function openHandoverPicker(query = '') {
        ...
        dialog.add(SelectCreateDialog, {
            ...
            multiSelect: false,
            context: {
                list_view_ref: 'aypatech_ai_assistant.ai_view_res_users_list_handover',
                kanban_view_ref: 'aypatech_ai_assistant.ai_view_res_users_kanban_handover',
                search_view_ref: 'aypatech_ai_assistant.ai_view_res_users_search_handover',
            },
            onSelected: (resIds) => {
                if (resIds && resIds[0]) {
                    onHandover(resIds[0]);
    ...
    async function setAgent(agentId, agentName = '') {
        ...
        try {
            await orm.write('aypatech_ai.session', [state.sessionId], {
    ...
    async function onRemoveAttachment(attachmentId) {
        ...
        try {
            await orm.call('aypatech_ai.session', 'discard_attachments', [
                state.sessionId,
                [attachmentId],
            ]);
        } catch {
    ...
    onWillUnmount(() => {
        ...
        bus.unsubscribe('aypatech_ai_assistant.ai_event', busHandler);
        clearStreamIdleTimer();
    });
    const api = {
        state,
