        ...
        if (turn.role === 'user') {
            ...
            if (text) {
                entries.push({
                    ...
                    role: 'user',
                    text,
                    anchorId: `ayp_msg_${turnIndex}_user`,
                });
            }
            ...
            for (let blockIndex = 0; blockIndex < blocks.length; blockIndex++) {
                ...
                entries.push({
                    ...
                    role: 'assistant',
                    text,
                    anchorId: `ayp_msg_${turnIndex}_${blockIndex}`,
                });
            }
        }
    }
...
function _wrapTextNode(
    ...
    while (from <= lower.length) {
        ...
        mark.setAttribute('class', isActive ? 'ayp_search_hit_active' : 'ayp_search_hit');
        mark.appendChild(
...
export function highlightHtml(html, query, activeMatchIdx, entryFirstMatchIdx) {
    ...
    const doc = parser.parseFromString(
        `<div id="__ayp_root__">${html || ''}</div>`,
        'text/html',
    );
    const root = doc.getElementById('__ayp_root__');
    if (!root) return html;
