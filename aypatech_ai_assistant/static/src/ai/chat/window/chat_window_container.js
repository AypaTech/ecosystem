...
import { ChatWindow } from '@aypatech_ai_assistant/ai/chat/window/chat_window';
import { dockWidth } from '@aypatech_ai_assistant/ai/chat/window/chat_window_service';
...
export class ChatWindowContainer extends Component {
    static template = 'aypatech_ai_assistant.ai_ChatWindowContainer';
    static components = { ChatWindow };
    static props = {};
    setup() {
        this.cw = useService('aypatech_ai_assistant.ai_chat_window');
     ...
     * from what it actually shows — `opened` stays populated while the
     * Discuss app is active and mail renders nothing, and a compact hub only
     * shows bubbles — so a workspace that never opens a chat keeps the dock
     * flush right.
     * @returns {number} reserved width in pixels
     */
    get discussReservedWidth() {
        const hub = this.mailStore.chatHub;
        if (this.mailStore.discuss?.isActive || (!hub.opened.length && !hub.folded.length)) {
            return 0;
        }
        const shown = hub.compact ? [] : hub.opened;
...
registry.category('main_components').add('aypatech_ai_assistant.ai_ChatWindowContainer', {
