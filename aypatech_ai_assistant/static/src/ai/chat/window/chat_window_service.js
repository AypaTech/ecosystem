...
function ensurePrism() {
    if (!prismPromise) {
        prismPromise = Promise.all([
            loadJS('/web/static/lib/prismjs/prism.js'),
            loadCSS('/web/static/lib/prismjs/themes/default.css'),
        ]).catch(() => {});
    }
    return prismPromise;
}
...
export const chatWindowService = {
    ...
    start(env) {
        ...
        env.services.bus_service.subscribe('aypatech_ai_assistant.ai_session_state', (payload) => {
...
registry.category('services').add('aypatech_ai_assistant.ai_chat_window', chatWindowService);
