...
import { SpaceDialog } from '@aypatech_ai_assistant/ai/chat/sidebar/space_dialog';

const DAY_MS = 24 * 60 * 60 * 1000;
const WIDTH_KEY = 'aypatech_ai_assistant.ai_sidebar_width';
const WIDTH_MIN = 220;
const WIDTH_MAX = 520;
...
export class ChatSidebar extends Component {
    static template = 'aypatech_ai_assistant.ai_ChatSidebar';
    props = useProps({
        sessions: t.array(),
        spaces: t.array().optional(),
        spaceSessions: t.object().optional(),
        spaceUnread: t.object().optional(),
        unreadIds: t.array().optional(),
        activeSessionId: t.or([t.number(), t.literal(null)]).optional(),
        hasMore: t.boolean().optional(),
        loadingMore: t.boolean().optional(),
        searching: t.boolean().optional(),
        searchMode: t.boolean().optional(),
        onNew: t.function(),
        onSelect: t.function(),
        onRename: t.function(),
        onDelete: t.function(),
        onQuery: t.function().optional(),
        onLoadMore: t.function().optional(),
        onSpaceOpen: t.function().optional(),
        onSpaceCreate: t.function().optional(),
        onSpaceEdit: t.function().optional(),
        onSpaceDelete: t.function().optional(),
        onSpaceReorder: t.function().optional(),
        onSpaceNew: t.function().optional(),
        onSpaceLoadMore: t.function().optional(),
        onSessionFile: t.function().optional(),
    });
    setup() {
        ...
        this.asideRef = signal.ref();
        this.listRef = signal.ref();
        this.spacesRef = signal.ref();
        this.newSpaceRef = signal.ref();
        useSortable({
            ref: this.spacesRef,
            elements: '.ayp_space_personal',
            handle: '.ayp_space_grip',
            cursor: 'grabbing',
            onDrop: ({ element, previous }) => this.onSpaceDrop(element, previous),
        });
        useSortable({
            ref: this.listRef,
            elements: '.ayp_sidebar_item:not(.ayp_sidebar_shared)',
            groups: '.ayp_space_group',
            connectGroups: true,
            cursor: 'grabbing',
            placeholderClasses: ['ayp_drag_placeholder'],
            onGroupEnter: ({ group }) => {
    ...
    onResizeStart(ev) {
        ...
        const startWidth = this.asideRef().getBoundingClientRect().width;
    ...
    startCreate(ev) {
        ...
        this.state.newName = '';
        requestAnimationFrame(() => this.newSpaceRef() && this.newSpaceRef().focus());
    }
    cancelCreate() {
