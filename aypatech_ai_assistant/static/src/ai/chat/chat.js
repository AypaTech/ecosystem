import { Component, markup, onMounted, onPatched, onWillStart, onWillUnmount, proxy, signal, useProps } from '@odoo/owl';
...
import { toFileModel, toInlineImageFile } from '@aypatech_ai_assistant/ai/core/attachment/attachment';
import { AttachmentCard } from '@aypatech_ai_assistant/ai/core/attachment/attachment_card';
import { AIChatShareUsers } from '@aypatech_ai_assistant/ai/chat/share/share_users';
import { useNotificationBadge } from '@aypatech_ai_assistant/ai/core/notification_badge';
import {
    approvalPill,
    costTooltip,
    formatCost,
    ...
    statusIcon,
    statusLabel,
} from '@aypatech_ai_assistant/ai/chat/utils';

import { ChatComposer } from '@aypatech_ai_assistant/ai/chat/composer/chat_composer';
import { useAiSession } from '@aypatech_ai_assistant/ai/chat/session/use_ai_session';
import {
    onScrollUpNearTop,
    preserveAnchor,
    useChatScrollAnchor,
} from '@aypatech_ai_assistant/ai/chat/session/use_scroll_anchor';
import { ChatSidebar } from '@aypatech_ai_assistant/ai/chat/sidebar/chat_sidebar';
import { ChatArtifactsPanel } from '@aypatech_ai_assistant/ai/chat/artifacts/chat_artifacts_panel';
import { SourceIcon, SourceList } from '@aypatech_ai_assistant/ai/chat/artifacts/types/sources_tab';
import '@aypatech_ai_assistant/ai/chat/artifacts/types/attachments_type';
import '@aypatech_ai_assistant/ai/chat/artifacts/types/sources_type';
import { ChatSearch } from '@aypatech_ai_assistant/ai/chat/search/chat_search';
import {
    buildIndex,
    ...
    findMatches,
    highlightHtml,
} from '@aypatech_ai_assistant/ai/chat/search/search_index';
import { ToolCard } from '@aypatech_ai_assistant/ai/chat/tools/tool_card';
import { ToolGroup, buildTurnItems } from '@aypatech_ai_assistant/ai/chat/tools/tool_group';
import {
    askArgsText,
    askViewMode,
    toggleAskViewMode,
} from '@aypatech_ai_assistant/ai/chat/session/ask_view';
import {
    viewContextLabel,
    viewContextTooltip,
} from '@aypatech_ai_assistant/ai/chat/session/view_context_format';
...
export class AIChat extends Component {
    static template = 'aypatech_ai_assistant.ai_Chat';
    static components = {
        ChatSidebar,
        ChatArtifactsPanel,
        ChatSearch,
    ...
    setup() {
        ...
        this.chatWindow = useService('aypatech_ai_assistant.ai_chat_window');
    ...
    _installImageClickHandler() {
        ...
        root.addEventListener('click', (ev) => {
            const img = ev.target.closest('.ayp_md_image');
    ...
    _installRootPasteHandler() {
        ...
        this._rootPasteHandler = (ev) => {
            ...
            if (ev.target && ev.target.closest('.ayp_composer textarea')) return;
    ...
    async _loadSpaces() {
        const [spaces, generalDomain] = await Promise.all([
            this.orm.call('aypatech_ai.space', 'fetch_spaces', []),
            this.orm.call('aypatech_ai.space', 'fetch_general_domain', []),
        ]);
     ...
     *
     * @returns {Array} a search domain on `aypatech_ai.session`
     */
    ...
    async onNewSession(values = {}) {
        ...
        let id;
        try {
            const sessionId = await this.orm.create('aypatech_ai.session', [
                { name, ...values },
            ]);
    ...
    async onRenameSession(sessionId, name) {
        await this.orm.write('aypatech_ai.session', [sessionId], { name });
        await this._loadSessions();
        if (this.session.state.sessionId === sessionId) {
    ...
    async onDeleteSession(sessionId) {
        await this.orm.unlink('aypatech_ai.session', [sessionId]);
    ...
    async onSpaceCreate(name) {
        await this.orm.create('aypatech_ai.space', [{ name }]);
        await this._loadSpaces();
    }
    async onSpaceEdit(spaceId, values) {
        await this.orm.write('aypatech_ai.space', [spaceId], values);
        await this._loadSpaces();
    }
    async onSpaceDelete(spaceId) {
        await this.orm.unlink('aypatech_ai.space', [spaceId]);
    ...
    async onSpaceReorder(spaceId, afterId) {
        ...
        try {
            await this.orm.call('aypatech_ai.space', 'reorder', [ids]);
        } catch (error) {
            await this._loadSpaces();
            throw error;
        }
    ...
    async onSessionFile(sessionId, spaceId) {
        ...
        await this.orm.write('aypatech_ai.session', [sessionId], {
    ...
    _handleScrollTarget() {
        ...
        if (el) {
            ...
            el.classList.add('ayp_search_pulse');
            window.setTimeout(() => {
                el.classList.remove('ayp_search_pulse');
            }, 600);
        }
        this.state.scrollTarget = null;
    }
    ...
    onPopout() {
        ...
        if (this.ui.isSmall) {
            this.action.doAction('aypatech_ai_assistant.ai_action_ai_chat', {
    ...
    _connectUserBus() {
        ...
        this.bus.subscribe('aypatech_ai_assistant.ai_session_state', this._userBusHandler);
    }
    _disconnectUserBus() {
        if (this._userBusHandler) {
            this.bus.unsubscribe('aypatech_ai_assistant.ai_session_state', this._userBusHandler);
            this._userBusHandler = null;
        }
    }
    ...
    get contextClass() {
        ...
        if (pct >= 90) return 'ayp_context_red';
        if (pct >= 70) return 'ayp_context_amber';
        return 'ayp_context_green';
    }
    get costPill() {
...
registry.category('actions').add('aypatech_ai_assistant.ai_chat', AIChat);
