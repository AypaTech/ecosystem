...
function buildRenderer() {
    ...
    md.renderer.rules.fence = (tokens, idx) => {
        ...
        return (
            ...
            `<button type="button" class="ayp_code_copy" aria-label="Copy code">Copy</button>` +
            `<code${langClass}>${body}</code>` +
            `</pre>`
        );
    };
    const TASK_RE = /^\[([ xX])\]\s+/;
    md.core.ruler.after('inline', 'aypatech_ai_task_lists', (state) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            tokens[i - 2].attrJoin('class', 'ayp_md_task');
        }
    });
    const LINKABLE_RE =
        /(\/web\/content\/\d+(?:\?[A-Za-z0-9_=&%.-]*)?)|\b([a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+),(\d+)\b/g;
    md.core.ruler.after('inline', 'aypatech_ai_links', (state) => {
        const tokens = state.tokens;
        for (const tok of tokens) {
            ...
            for (const child of tok.children) {
                ...
                while ((match = LINKABLE_RE.exec(text)) !== null) {
                    ...
                    lo.attrSet('class', fileUrl ? 'ayp_file_link' : 'ayp_record_link');
                    newChildren.push(lo);
    ...
    md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
        ...
        if (hrefIdx >= 0) {
            ...
            if (record) {
                ...
                token.attrJoin('class', 'ayp_record_link');
            } else {
    ...
    md.renderer.rules.image = (tokens, idx, options, env, self) => {
        ...
        token.attrSet('class', 'ayp_md_image');
        return self.renderToken(tokens, idx, options, env);
    };
    return md;
}
...
if (typeof window !== 'undefined' && !window.__mkCopyBound) {
    ...
    document.addEventListener('click', (ev) => {
        const btn = ev.target.closest && ev.target.closest('.ayp_code_copy');
        if (!btn) {
            return;
        }
        ...
        const reset = () => {
            ...
            btn.classList.remove('ayp_code_copy_done', 'ayp_code_copy_fail');
        };
        navigator.clipboard
            .writeText(code.textContent)
            .then(() => {
                btn.textContent = 'Copied';
                btn.classList.add('ayp_code_copy_done');
                setTimeout(reset, 1500);
            })
            .catch(() => {
                btn.textContent = 'Failed';
                btn.classList.add('ayp_code_copy_fail');
                setTimeout(reset, 1500);
            });
    });
}
