...
from odoo.addons.aypatech_ai_assistant.schedule.tools.constants import (
    DEFAULT_MAX_COST_EUR,
    DEFAULT_MAX_LIFETIME_HOURS,
    DEFAULT_MAX_RESUMES,
    DEFAULT_MAX_TOTAL_TOKENS,
)
from odoo.addons.aypatech_ai_assistant.schedule.tools.recurrence import compute_next_call


class AISchedule(models.Model):
    """Fire AI agent sessions on a cron cadence through an owned action."""

    _name = 'aypatech_ai.schedule'
    ...
    def _default_model_id(self) -> int:
        ...
        return self.env.ref('aypatech_ai_assistant.model_aypatech_ai_session').id
    ...
    def _assert_record_code_access(self, vals: dict) -> None:
        ...
        the admin-only gate ``aypatech_ai_automation`` enforces on direct agent
    ...
    def action_fire_now(self) -> dict:
        ...
        Session = self.env['aypatech_ai.session'].sudo()
