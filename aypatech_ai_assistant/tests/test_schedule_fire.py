...
from .common_schedule import ScheduleTestCommon
from odoo.addons.aypatech_ai_assistant.automation.tools.dispatch import _resolve_records


@tagged('post_install', '-at_install', 'aypatech_ai_schedule')
class TestScheduleFire(ScheduleTestCommon):
    ...
    def _fire(self, schedule: models.BaseModel) -> models.BaseModel:
        ...
        return self.env['aypatech_ai.session'].search(
            [
                ('schedule_id', '=', schedule.id),
    ...
    def test_action_fire_now_wires_to_dispatch(self):
        schedule = self._make_schedule()
        before_count = self.env['aypatech_ai.session'].search_count(
            [('schedule_id', '=', schedule.id)],
        )
        with self._mock_provider():
            action = schedule.action_fire_now()
        after_count = self.env['aypatech_ai.session'].search_count(
            [('schedule_id', '=', schedule.id)],
        )
    ...
    def test_action_fire_now_owner_opens_chat(self):
        ...
        spawned = self.env['aypatech_ai.session'].search(
            [('schedule_id', '=', schedule.id)],
            limit=1,
        )
    ...
    def test_render_failure_posts_prompt_render_error_event(self):
        ...
        with self._mock_provider():
            sessions = self._fire(schedule)
        self.assertEqual(len(sessions), 1)
        events = self.env['aypatech_ai.session.event'].search(
            [
