...
class TestMentionDiscuss(ChatterTestCommon):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.chat = cls.env['discuss.channel'].channel_get(
    ...
    @mute_logger('odoo.addons.aypatech_ai_assistant.chatter.models.mail_thread')
    def test_a_mention_over_the_session_quota_still_posts_the_message(self):
        provider = self.env['aypatech_ai.provider']._get_default().sudo()
        ...
        self.env['aypatech_ai.session'].with_user(poster).create({'name': 'Quota Filler'})
        with self._mute_worker():
