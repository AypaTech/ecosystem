...
from odoo.addons.aypatech_ai_assistant.ai.models import ir_http as ir_http_module
from odoo.addons.aypatech_ai_assistant.ai.models import session as session_module
from odoo.addons.aypatech_ai_assistant.tests.common import AITestCommon, FakeRequest
from odoo.addons.aypatech_ai_assistant.ai.tools import ADVISORY_LOCK_NAMESPACE, DISPATCH_MAX_TURNS
...
class TestTurnDispatch(AITestCommon):
    ...
    def _resolve_inline(
        ...
        :param mode: value stored in the ``aypatech_ai_assistant.ai_dispatch_mode`` parameter
        """
        self.env['ir.config_parameter'].sudo().set_param('aypatech_ai_assistant.ai_dispatch_mode', mode)
        ...
        with (
            ...
            return self.env['aypatech_ai.session']._dispatch_inline()
    ...
    def _budget(
        ...
        :param method: name of the ``aypatech_ai.session`` budget method to call
        ...
        try:
            with patch.object(session_module, 'config', limits):
                return getattr(self.env['aypatech_ai.session'], method)()
        finally:
    ...
    def _trigger(
        ...
        with (
            ...
            patch.object(
                type(self.env['aypatech_ai.session']),
    ...
    def test_dispatch_stays_on_cron_during_tests(self):
        self.assertFalse(self.env['aypatech_ai.session']._dispatch_inline())
    ...
    def test_queue_is_skipped_without_a_request(self):
        session = self.env['aypatech_ai.session'].create({'name': 'no-request'})
        unbound = FakeRequest(bound=False)
        self._trigger(session, unbound)
        self.assertFalse(hasattr(unbound, 'aypatech_ai_dispatch_ids'))

    def test_queue_collects_sessions_in_order_without_duplicates(self):
        first = self.env['aypatech_ai.session'].create({'name': 'first'})
        second = self.env['aypatech_ai.session'].create({'name': 'second'})
        fake_request = FakeRequest()
        self._trigger(first + second, fake_request)
        self._trigger(first, fake_request)
        self.assertEqual(fake_request.aypatech_ai_dispatch_ids, (first.id, second.id))

    def test_queue_is_skipped_when_dispatch_stays_on_cron(self):
        session = self.env['aypatech_ai.session'].create({'name': 'cron-only'})
        fake_request = FakeRequest()
        self._trigger(session, fake_request, inline=False)
        self.assertFalse(getattr(fake_request, 'aypatech_ai_dispatch_ids', ()))

    def test_slot_is_claimed_and_released_on_the_real_cursor(self):
        session_model = self.env['aypatech_ai.session']
    ...
    def test_slot_is_released_even_when_the_turn_raises(self):
        with (
            ...
            patch.object(
                type(self.env['aypatech_ai.session']), '_release_dispatch_slot'
            ) as released,
            patch.object(
                type(self.env['aypatech_ai.session']),
            ...
            self.env['aypatech_ai.session']._dispatch_in_slot((1,))
    ...
    def test_queued_turns_are_dispatched_in_order(self):
        ...
        second = self.env['aypatech_ai.session'].create({'name': 'second'})
        with patch.object(
            type(self.env['aypatech_ai.session']), '_process_session_in_worker'
        ) as worker:
            self.env['aypatech_ai.session']._dispatch_queued_turns((first.id, second.id))
        self.assertEqual(
