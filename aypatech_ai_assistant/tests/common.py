...
from odoo.addons.aypatech_ai_assistant.ai.tools.url_fetch import FetchResult
...
class RegistryTestModeMixin:
    """Backport of ``enter_registry_test_mode`` from the 19.0 test cases.

    Code under test that opens its own cursor (workers, crons, loggers) must
    see the data of the running test, so every new cursor reuses the test one
...
class AITestCommon(RegistryTestModeMixin, TransactionCase):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.provider = cls.env.ref('aypatech_ai_assistant.ai_provider_openai')
        cls.provider.sudo().write({'api_key': 'test-key', 'active': True})
        cls.provider_anthropic = cls.env.ref('aypatech_ai_assistant.ai_provider_anthropic')
        cls.provider_anthropic.sudo().api_key = 'test-key'
        cls.provider_google = cls.env.ref('aypatech_ai_assistant.ai_provider_google')
