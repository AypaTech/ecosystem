...
from odoo.addons.aypatech_ai_assistant.ai.models import ir_attachment as ir_att
from odoo.addons.aypatech_ai_assistant.ai.providers.anthropic import AnthropicProvider
from odoo.addons.aypatech_ai_assistant.ai.providers.openai import OpenAIProvider
...
class TestMultimodalAttachments(AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.session = cls.env['aypatech_ai.session'].create({'name': 'Multimodal'})
    ...
    def test_materialize_image(self):
        ...
        self.assertEqual(block['type'], 'aypatech_ai_attachment')
    ...
    def test_upload_attachments(self):
        ...
        self.assertEqual(attachment.res_model, 'aypatech_ai.session')
    ...
    def test_unlink_session_cleans_attachments(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Disposable'})
    ...
    def test_send_message_carries_attachment(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Send'})
        ...
        attachment_blocks = [b for b in blocks if b['type'] == 'aypatech_ai_attachment']
    ...
    def test_send_message_attachment_only(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Attach-only'})
        attachment = self._make_attachment('pic.png', 'image/png', PNG_BYTES)
        with self._mock_responses([self._make_text_response('done')]):
        ...
        self.assertEqual(blocks[0]['type'], 'aypatech_ai_attachment')

    def test_resolve_rejects_unrelated_ids(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Resolve'})
        with self.assertRaises(UserError):
            session._resolve_attachments([999999999])

    def test_resolve_rejects_foreign_attachment(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Foreign'})
    ...
    def test_provider_materializes_attachment_blocks(self):
        ...
        self.assertEqual(blocks[1]['type'], 'aypatech_ai_attachment')
