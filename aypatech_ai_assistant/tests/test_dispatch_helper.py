...
from .common_automation import AutomationTestCommon
from odoo.addons.aypatech_ai_assistant.automation.tools.constants import MAX_PROMPT_CHARS
from odoo.addons.aypatech_ai_assistant.automation.tools.dispatch import (
    PreviousProxy,
    _build_prompt,
    _create_session,
    _resolve_records,
...
@tagged('post_install', '-at_install', 'aypatech_ai_automation')
class TestDispatchHelper(AutomationTestCommon):
    ...
    def test_previous_proxy_tool_log_with_logged_session(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Logged Session'})
