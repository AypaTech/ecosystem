...
class TestAiModel(AITestCommon):
    ...
    def test_compute_usage_cost_zero_on_empty(self):
        empty = self.env['aypatech_ai.model'].browse([])
    ...
    def test_provider_default_model_domain_scopes_to_own_models(self):
        ...
        domain = safe_eval(
            self.env['aypatech_ai.provider']._fields['default_model_id'].domain,
            {'id': self.provider.id},
        )
        selectable = self.env['aypatech_ai.model'].search(domain)
    ...
    def test_unique_provider_model_is_scoped_to_the_provider(self):
        ...
        self.assertEqual(
            self.env['aypatech_ai.model'].search_count(
