...
class TestLogUnification(AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.session = cls.env['aypatech_ai.session'].create({'name': 'log-unification'})
    ...
    def test_chat_call_writes_log_row_with_source_chat_and_session_id(self):
        ...
        self.assertTrue(ok)
        rows = self.env['aypatech_mcp.log'].search(
            [
    ...
    def test_external_call_writes_row_with_source_mcp_and_no_session(self):
        with self._patch_execute({'list_modules': '{}'}):
            self.env['aypatech_mcp.tool']._call(
                'list_modules',
                {},
                self.env,
                enforce_scope=None,
    ...
    def test_cascade_delete(self):
        session = self.env['aypatech_ai.session'].create({'name': 'cascade'})
        with self._patch_execute({'list_modules': '{}'}):
        ...
        row_ids = rows.ids
        session.unlink()
        survivors = self.env['aypatech_mcp.log'].search(
            [
                ('id', 'in', row_ids),
            ]
        )
    ...
    def test_unified_events_reads_from_events(self):
        session = self.env['aypatech_ai.session'].create({'name': 'merge'})
    ...
    def test_tool_calls_persisted_to_events(self):
        session = self.env['aypatech_ai.session'].create({'name': 'jsonb'})
        with self._patch_execute({'list_modules': '{}'}):
    ...
    def test_clear_preserves_events_and_appends_marker(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clear-events'})
        session._append_event(
            {'kind': 'user_message', 'content': 'hi', 'attachments': []}
        )
    ...
    def test_compact_drops_event_rows(self):
        session = self.env['aypatech_ai.session'].create({'name': 'compact-events'})
        session._append_event(
            {'kind': 'user_message', 'content': 'hi', 'attachments': []}
        )
        with self._patch_execute({'list_modules': '{}'}):
    ...
    def test_regenerate_truncates_to_last_user_message(self):
        session = self.env['aypatech_ai.session'].create({'name': 'regen-events'})
        session._append_event(
    ...
    def test_reject_writes_denied_audit_row(self):
        session = self.env['aypatech_ai.session'].create({'name': 'reject-audit'})
        session.pending_ask = {
            'kind': 'approval',
            'call_id': 'call-1',
            'name': 'delete_records',
    ...
    def test_clear_then_new_tool_call_visible_after_reload(self):
        session = self.env['aypatech_ai.session'].create({'name': 'post-clear'})
    ...
    def test_chat_audit_respects_global_mcp_logging(self):
        ...
        session = self.env['aypatech_ai.session'].create({'name': 'audit-disabled'})
        before = (
            self.env['aypatech_mcp.log']
            .sudo()
            .search_count(
    ...
    def test_unified_events_returns_latest_window(self):
        session = self.env['aypatech_ai.session'].create({'name': 'pag-window'})
    ...
    def test_unified_events_before_sequence_paginates_older(self):
        session = self.env['aypatech_ai.session'].create({'name': 'pag-older'})
        for index in range(250):
            session._append_event(
                {
                    'kind': 'text',
    ...
    def test_unified_events_below_limit_no_more(self):
        session = self.env['aypatech_ai.session'].create({'name': 'pag-small'})
    ...
    def test_clear_session_preserves_event_history(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clear-trunc'})
