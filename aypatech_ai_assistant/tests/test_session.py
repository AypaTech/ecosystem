...
from odoo.addons.aypatech_ai_assistant.ai.models import session as session_module
from odoo.addons.aypatech_ai_assistant.tests.common import AITestCommon
from odoo.addons.aypatech_ai_assistant.ai.tools import (
...
class TestAiSession(AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.session = cls.env['aypatech_ai.session'].create({'name': 'Test session'})
    ...
    def _patch_tool_call(
        ...
        tool_model = self.env['aypatech_mcp.tool']
        return patch.object(
            type(tool_model),
            '_execute',
            autospec=True,
    ...
    def _force_stale_session(
        ...
        session = self.env['aypatech_ai.session'].create({'name': name})
    ...
    def _backdate_session(self, session: models.BaseModel) -> None:
        ...
        self.env.cr.execute(
            SQL(
                'UPDATE aypatech_ai_session SET write_date = %s, claimed_at = %s '
                'WHERE id = %s',
                stale,
                stale,
                session.id,
    ...
    def _inbox_session(self, name: str = 'inbox session') -> models.BaseModel:
        ...
        return self.env['aypatech_ai.session'].with_user(user).create({'name': name})
    ...
    def test_max_iterations_is_configurable(self):
        self.env['ir.config_parameter'].sudo().set_param('aypatech_ai_assistant.ai_max_iterations', '3')
        session = self.env['aypatech_ai.session'].create({'name': 'cfg iters'})
        payloads = [
    ...
    def test_int_config_param_falls_back_on_invalid(self):
        ...
        ICP.set_param('aypatech_ai_assistant.ai_max_iterations', '7')
        self.assertEqual(self.session._max_iterations(), 7)
        ICP.set_param('aypatech_ai_assistant.ai_max_iterations', 'nonsense')
        self.assertEqual(self.session._max_iterations(), MAX_ITERATIONS)
        ICP.set_param('aypatech_ai_assistant.ai_max_iterations', '0')
        self.assertEqual(self.session._max_iterations(), MAX_ITERATIONS)

    def test_slice_wallclock_bounded_by_cron_limit(self):
        ICP = self.env['ir.config_parameter'].sudo()
        ICP.set_param('aypatech_ai_assistant.ai_slice_wallclock_seconds', '99999')
        ...
        ICP.set_param('aypatech_ai_assistant.ai_slice_wallclock_seconds', '15')
    ...
    def test_new_user_turn_resets_turn_wallclock_spent(self):
        session = self.env['aypatech_ai.session'].create({'name': 'reset turn'})
        session.write(
            {
                'state': 'done',
    ...
    def test_get_tool_schema_converts_inputschema_to_parameters(self):
        ...
        with patch.object(
            type(self.env['aypatech_mcp.tool']),
            'get_tools',
            autospec=True,
    ...
    def test_bus_event_published_on_state_change(self):
        ...
        states = [
            ...
            if notification_type == 'aypatech_ai_assistant.ai_session_state' and target == partner
        ]
        ...
        events = [
            message
            for target, notification_type, message in captured
            if notification_type == 'aypatech_ai_assistant.ai_event' and target == self.session
        ]
    ...
    def test_state_notification_payload_is_timestamped(self):
        ...
        payloads = [
            ...
            if channel == 'aypatech_ai_assistant.ai_session_notification'
        ]
    ...
    def test_email_pref_user_is_flagged_but_not_notified(self):
        ...
        session = (
            self.env['aypatech_ai.session'].with_user(user).create({'name': 'email pref'})
        )
        session._notify_state_transition({'state': 'done'})
    ...
    def test_skipped_done_transition_leaves_session_unflagged(self):
        ...
        self.session.with_context(
            aypatech_ai_skip_done_notification=True
    ...
    def test_notification_badge_counts_unread_sessions(self):
        Session = self.env['aypatech_ai.session']
    ...
    def test_state_transition_pushes_badge(self):
        ...
        with patch.object(
            ...
            self.session._notify_state_transition({'state': 'done'})
        self.assertIn('aypatech_ai_assistant.ai_notification_badge', captured)

    def test_unlink_broadcasts_deletion(self):
        session = self.env['aypatech_ai.session'].create({'name': 'to delete'})
        ...
        deletions = [
            ...
            if ntype == 'aypatech_ai_assistant.ai_session_state' and message.get('deleted')
        ]
    ...
    def test_ask_user_skips_sibling_tool_calls(self):
        ...
        with (
            ...
            snapshot = self.env['aypatech_ai.session'].create({'name': 'race'}).start('go')
        self.assertEqual(snapshot['state'], 'waiting')
        self.assertNotIn('write_record', dispatched)

    def test_effective_model_uses_agent_override(self):
        model = self.env['aypatech_ai.model'].create(
            {
                'name': 'Big',
        ...
        agent = self.env['aypatech_ai.agent'].create(
            {
                'name': 'Big',
                'model_id': model.id,
            }
        )
        session = self.env['aypatech_ai.session'].create(
            {
                'name': 'Big session',
                'agent_id': agent.id,
            }
    ...
    def test_effective_model_falls_back_to_global_default(self):
        fallback = self.env['aypatech_ai.model'].create(
            {
                'name': 'Fallback',
                'provider_id': self.provider.id,
        ...
        session = self.env['aypatech_ai.session'].create({'name': 'No override'})
    ...
    def test_terminating_tool_triggers_summary_and_stops(self):
        session = self.env['aypatech_ai.session'].create({'name': 'terminating'})
        open_view_result = (
    ...
    def test_terminating_tool_skips_sibling_tool_calls(self):
        ...
        with (
            ...
            snapshot = self.env['aypatech_ai.session'].create({'name': 'skip'}).start('go')
        self.assertEqual(snapshot['state'], 'done')
    ...
    def test_agent_capability_flags_default_off(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Plain'})
        session = self.env['aypatech_ai.session'].create(
            {
                'name': 'default flags',
                'agent_id': agent.id,
    ...
    def test_last_input_tokens_tracks_most_recent_turn(self):
        session = self.env['aypatech_ai.session'].create({'name': 'token meter'})
    ...
    def test_clear_resets_conversation(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clearable'})
    ...
    def test_clear_refuses_running(self):
        session = self.env['aypatech_ai.session'].create({'name': 'running'})
    ...
    def test_compact_replaces_conversation_with_summary(self):
        session = self.env['aypatech_ai.session'].create({'name': 'compactable'})
    ...
    def test_auto_compact_skips_when_the_kept_tail_alone_fills_the_window(self):
        ...
        agent = self.env['aypatech_ai.agent'].create({'name': 'Tiny', 'model_id': model.id})
        session = self.env['aypatech_ai.session'].create(
    ...
    def test_compact_refuses_empty(self):
        session = self.env['aypatech_ai.session'].create({'name': 'empty'})
        with self.assertRaises(UserError):
            session.compact()

    def test_compact_preserves_recent_tail(self):
        session = self.env['aypatech_ai.session'].create({'name': 'tail-keep'})
    ...
    def test_compact_uses_anchored_update_when_prior_summary_exists(self):
        session = self.env['aypatech_ai.session'].create({'name': 'anchored'})
    ...
    def test_compact_keeps_event_log(self):
        session = self.env['aypatech_ai.session'].create({'name': 'eventlog'})
        self._seed_large_conversation(session)
        events_before = session.fetch_events(limit=500)['events']
        self.assertTrue(events_before)
    ...
    def test_compact_includes_available_tools_block(self):
        session = self.env['aypatech_ai.session'].create({'name': 'tools-block'})
        self._seed_large_conversation(session)
        with self._patch_provider([self._text_payload('compact summary')]):
            session.compact()
    ...
    def test_compact_accrues_cost(self):
        session = self.env['aypatech_ai.session'].create({'name': 'cost-accrue'})
        self._seed_large_conversation(session)
    ...
    def test_compact_empty_summary_rollback(self):
        session = self.env['aypatech_ai.session'].create({'name': 'empty-summary'})
        self._seed_large_conversation(session)
    ...
    def test_view_context_injected_into_inputs(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx'})
    ...
    def test_view_context_not_persisted_in_conversation(self):
        session = self.env['aypatech_ai.session'].create({'name': 'nopersist'})
        session.view_context = {
            'kind': 'record',
            'model': 'res.partner',
            'id': 7,
    ...
    def test_view_context_updates_on_open_record_tool(self):
        session = self.env['aypatech_ai.session'].create({'name': 'open-record'})
    ...
    def test_view_context_updates_on_open_view_tool(self):
        session = self.env['aypatech_ai.session'].create({'name': 'open-view'})
        open_view_result = (
            '{"type": "ir.actions.act_window", "res_model": "res.partner", '
    ...
    def test_set_view_context_record(self):
        session = self.env['aypatech_ai.session'].create({'name': 'set'})
    ...
    def test_set_view_context_clears_on_none(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clear-ctx'})
        session.view_context = {
            'kind': 'record',
            'model': 'res.partner',
            'id': 3,
    ...
    def test_set_view_context_rejects_bad_payload(self):
        session = self.env['aypatech_ai.session'].create({'name': 'bad'})
        with self.assertRaises(UserError):
    ...
    def test_unpin_view_context(self):
        session = self.env['aypatech_ai.session'].create({'name': 'unpin'})
        session.view_context = {
            'kind': 'record',
            'model': 'res.partner',
            'id': 3,
    ...
    def test_regenerate_last_turn_rewinds_and_replays(self):
        session = self.env['aypatech_ai.session'].create({'name': 'rewind'})
    ...
    def test_regenerate_refuses_while_compacting(self):
        session = self.env['aypatech_ai.session'].create({'name': 'compacting'})
    ...
    def test_undo_to_event_truncates_log_and_conversation(self):
        session = self.env['aypatech_ai.session'].create({'name': 'rewindable'})
    ...
    def test_undo_to_event_on_assistant_keeps_preceding_user(self):
        session = self.env['aypatech_ai.session'].create({'name': 'rewind-asst'})
        with self._patch_provider([self._text_payload('answer 1')]):
            session.start('question 1')
        with self._patch_provider([self._text_payload('answer 2')]):
            session.send_message('question 2')
    ...
    def test_undo_after_answer_with_attachments_keeps_alignment(self):
        session = self.env['aypatech_ai.session'].create({'name': 'answer-attach'})
        with self._patch_provider(
    ...
    def test_request_inputs_strip_internal_keys(self):
        session = self.env['aypatech_ai.session'].create({'name': 'strip'})
        session.conversation = [
    ...
    def test_fork_at_event_creates_independent_session(self):
        session = self.env['aypatech_ai.session'].create({'name': 'forkable'})
        with self._patch_provider([self._text_payload('answer 1')]):
            session.start('question 1')
        with self._patch_provider([self._text_payload('answer 2')]):
            session.send_message('question 2')
        ...
        fork = self.env['aypatech_ai.session'].browse(fork_id)
    ...
    def test_fork_at_event_preserves_agent(self):
        agent = self.env['aypatech_ai.agent']._get_default()
        session = self.env['aypatech_ai.session'].create(
            {
                'name': 'fork-agent',
                'agent_id': agent.id,
    ...
    def test_fork_refuses_while_running(self):
        session = self.env['aypatech_ai.session'].create({'name': 'running-fork'})
        with self._patch_provider([self._text_payload('answer')]):
            session.start('question')
        target = session.event_ids.sorted('sequence')[0]
        session.write({'state': 'running'})
    ...
    def test_undo_after_clear_truncates_conversation(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clear-undo'})
        with self._patch_provider([self._text_payload('answer 1')]):
            session.start('question 1')
        session.clear()
        with self._patch_provider([self._text_payload('answer 2')]):
    ...
    def test_fork_after_clear_does_not_copy_tail(self):
        session = self.env['aypatech_ai.session'].create({'name': 'clear-fork'})
        with self._patch_provider([self._text_payload('answer 1')]):
            session.start('question 1')
        session.clear()
        with self._patch_provider([self._text_payload('answer 2')]):
        ...
        fork = self.env['aypatech_ai.session'].browse(session.fork_at_event(target.id))
        self.assertNotIn(
            'question 3',
    ...
    def test_compact_tail_drop_fallback_on_provider_error(self):
        session = self.env['aypatech_ai.session'].create({'name': 'tail-drop'})
    ...
    def test_upload_attachments_creates_and_links(self):
        session = self.env['aypatech_ai.session'].create({'name': 'upload'})
    ...
    def test_discard_attachments_unlinks_orphans(self):
        session = self.env['aypatech_ai.session'].create({'name': 'discard'})
        session.upload_attachments(
            [
                {
                    'filename': 'a.txt',
    ...
    def test_set_view_context_list_with_domain(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-list'})
    ...
    def test_set_view_context_action(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-action'})
        session.set_view_context(
            {
                'kind': 'action',
                'model': 'sale.order',
    ...
    def test_set_view_context_pivot_with_measures(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-pivot'})
        session.set_view_context(
            {
                'kind': 'pivot',
                'model': 'sale.order',
    ...
    def test_set_view_context_graph(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-graph'})
        session.set_view_context(
            {
                'kind': 'graph',
                'model': 'sale.order',
    ...
    def test_set_view_context_rejects_non_string_model(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-bad-model'})
        with self.assertRaises(UserError):
            session.set_view_context(
                {
                    'kind': 'list',
    ...
    def test_set_view_context_rejects_non_list_domain(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-bad-domain'})
        with self.assertRaises(UserError):
            session.set_view_context(
                {
                    'kind': 'list',
    ...
    def test_set_view_context_rejects_non_list_pivot_fields(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-pivot-bad'})
        with self.assertRaises(UserError):
            session.set_view_context(
                {
                    'kind': 'pivot',
    ...
    def test_set_view_context_rejects_non_list_graph_groupbys(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-graph-bad'})
        with self.assertRaises(UserError):
            session.set_view_context(
                {
                    'kind': 'graph',
    ...
    def test_set_view_context_rejects_non_string_graph_mode(self):
        session = self.env['aypatech_ai.session'].create({'name': 'ctx-graph-mode'})
        with self.assertRaises(UserError):
            session.set_view_context(
                {
                    'kind': 'graph',
    ...
    def test_set_approval_mode_rejects_unknown(self):
        session = self.env['aypatech_ai.session'].create({'name': 'mode-bad'})
        with self.assertRaises(UserError):
    ...
    def test_send_message_routes_first_turn_to_start(self):
        session = self.env['aypatech_ai.session'].create({'name': 'new'})
    ...
    def test_send_message_routes_pending_question_to_answer(self):
        session = self.env['aypatech_ai.session'].create({'name': 'pending'})
        with self._patch_provider(
            [
                self._tool_payload('ask_user', {'question': 'Which year?'}, 'call_q'),
            ]
    ...
    def test_enqueue_message_queues_while_running(self):
        session = self.env['aypatech_ai.session'].create({'name': 'queue-live'})
        session.write({'state': 'running'})
    ...
    def test_enqueue_message_rejects_when_turn_already_over(self):
        session = self.env['aypatech_ai.session'].create({'name': 'queue-late'})
    ...
    def test_recover_if_stuck_noop_for_fresh_session(self):
        session = self.env['aypatech_ai.session'].create({'name': 'fresh'})
        session.write({'state': 'running'})
        self.assertFalse(session._recover_if_stuck())
        session.invalidate_recordset()
        self.assertEqual(session.state, 'running')
    ...
    def test_sweep_orphan_closes_orphan_tool_calls(self):
        ...
        self._backdate_session(session)
        self.env['aypatech_ai.session']._sweep_orphan_sessions()
        session.invalidate_recordset()
        self.assertEqual(session.state, 'error')
        outputs = [
            entry
    ...
    def test_recover_if_stuck_preserves_pending_queue(self):
        ...
        self.env['aypatech_ai.session.pending'].create(
            {
                'session_id': session.id,
    ...
    def test_sweep_orphan_skips_fresh_session_with_recent_write_date(self):
        session = self.env['aypatech_ai.session'].create({'name': 'just-started'})
        session.write({'state': 'running'})
        self.env['aypatech_ai.session']._sweep_orphan_sessions()
        session.invalidate_recordset()
        self.assertEqual(session.state, 'running')
    ...
    def test_sweep_orphan_skips_session_with_recent_heartbeat(self):
        session = self.env['aypatech_ai.session'].create({'name': 'live-heartbeat'})
        session.write(
            {
                'state': 'running',
    ...
    def test_heartbeat_claim_bumps_claimed_at(self):
        session = self.env['aypatech_ai.session'].create({'name': 'beating'})
    ...
    def test_close_orphan_tool_calls_closes_unanswered_calls(self):
        session = self.env['aypatech_ai.session'].create({'name': 'orphan-calls'})
        session.conversation = [
            {'role': 'user', 'content': 'hi'},
            {
                'type': 'function_call',
    ...
    def test_split_conversation_tail_starts_at_user_turn(self):
        session = self.env['aypatech_ai.session'].create({'name': 'split-pairs'})
    ...
    def test_split_conversation_single_user_turn_keeps_it_verbatim(self):
        session = self.env['aypatech_ai.session'].create({'name': 'split-short'})
        conversation = [
    ...
    def test_process_tool_round_skips_second_ask_user(self):
        session = self.env['aypatech_ai.session'].create({'name': 'double-ask'})
        session.state = 'running'
        outputs = []
    ...
    def test_round_notice_warns_before_iteration_cliff(self):
        ...
        self.addCleanup(
            ...
            'aypatech_ai_assistant.ai_max_iterations',
            '',
        )
        captured = []
        with (
    ...
    def test_turn_cost_budget_stops_turn(self):
        self.env['ir.config_parameter'].sudo().set_param(
            'aypatech_ai_assistant.ai_turn_cost_limit', '0.01'
        )
        self.addCleanup(
            self.env['ir.config_parameter'].sudo().set_param,
            'aypatech_ai_assistant.ai_turn_cost_limit',
            '',
        )
