...
from odoo.addons.aypatech_ai_assistant.ai.tools import DEFAULT_CONTEXT_WINDOW
...
class TestAiAgent(AITestCommon):
    ...
    def _make_model(
        ...
        :param provider: Provider to attach the model to; defaults to
            ``self.provider``.
        """
        return self.env['aypatech_ai.model'].create(
            {
                'name': model_name,
    ...
    def test_default_from_company(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'CompanyPick'})
        self.env.company.default_ai_agent_id = agent
        self.assertEqual(self.env['aypatech_ai.agent']._get_default(), agent)

    def test_default_falls_back_to_any_active_agent(self):
        self.env.company.default_ai_agent_id = False
        active = self.env['aypatech_ai.agent'].search(
            [('active', '=', True)],
            limit=1,
        )
        self.assertTrue(active)
        self.assertEqual(self.env['aypatech_ai.agent']._get_default(), active)

    def test_default_skips_archived_company_pick(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Retired'})
        self.env.company.default_ai_agent_id = agent
        agent.active = False
        result = self.env['aypatech_ai.agent']._get_default()
        self.assertNotEqual(result, agent)
        self.assertTrue(result)

    def test_session_uses_default_agent(self):
        default = self.env['aypatech_ai.agent'].create({'name': 'Autopick'})
    ...
    def test_session_uses_company_default_agent_prompt(self):
        Agent = self.env['aypatech_ai.agent']
        ...
        session = self.env['aypatech_ai.session'].create({'name': 'No agent'})
        self.assertEqual(
    ...
    def test_render_eval_context_exposes_odoo_version(self):
        ctx = self.env['aypatech_ai.agent']._get_default()._prompt_eval_context()
    ...
    def test_runtime_block_contains_core_facts(self):
        session = self.env['aypatech_ai.session'].create({'name': 'Runtime'})
    ...
    def test_runtime_block_omits_multi_company_line_when_single(self):
        session = self.env['aypatech_ai.session'].create({'name': 'SingleCo'})
    ...
    def test_runtime_block_emits_multi_company_line_when_multi(self):
        ...
        extra = Company.create({'name': 'AypaTech Extra Test Co'})
        ...
        session = self.env['aypatech_ai.session'].create({'name': 'MultiCo'})
        block = session.with_user(multi)._build_runtime_block()
        self.assertIn('Companies accessible', block)
        self.assertIn('AypaTech Extra Test Co', block)
    ...
    def test_agent_resolve_context_window_falls_back_to_default(self):
        ...
        agent = self.env['aypatech_ai.agent'].create({'name': 'Auto'})
        self.assertEqual(agent._resolve_context_window(), 64000)

    def test_agent_resolve_context_window_hard_fallback(self):
        default_provider = self.env['aypatech_ai.provider']._get_default()
        default_provider.default_model_id = False
        agent = self.env['aypatech_ai.agent'].create({'name': 'NoModel'})
    ...
    def test_action_open_sessions_filters_by_agent(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Opener'})
        action = agent.action_open_sessions()
        self.assertEqual(action['type'], 'ir.actions.act_window')
        self.assertEqual(action['res_model'], 'aypatech_ai.session')
    ...
    def test_action_open_prompt_history_targets_system_prompt(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Historian'})
        action = agent.action_open_prompt_history()
        self.assertEqual(action['tag'], 'aypatech_ai_assistant.ai_prompt_history_dialog')
        self.assertEqual(action['params']['res_model'], 'aypatech_ai.agent')
    ...
    def test_compute_tool_filter_options_lists_the_catalog_sorted_by_name(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Options'})
    ...
    def test_compute_session_count_matches_created_sessions(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Counter'})
        self.env['aypatech_ai.session'].create({'name': 's1', 'agent_id': agent.id})
        self.env['aypatech_ai.session'].create({'name': 's2', 'agent_id': agent.id})
    ...
    def test_get_placeholder_filename_returns_icon_for_image_fields_only(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Img'})
        image_fields = (
            'image_1920',
            'image_1024',
            'image_512',
        ...
        for field in image_fields:
            self.assertEqual(
                ...
                'aypatech_ai_assistant/static/description/icon.png',
            )
