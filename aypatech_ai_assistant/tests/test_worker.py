...
@tagged('post_install', '-at_install', 'aypatech_ai')
class TestSessionWorker(AITestCommon):
    ...
    def _worker_action(self, name: str = 'AI worker') -> models.Model:
        ...
        return self.env['ir.actions.server'].create(
            {
                'name': name,
                'model_id': self.env['ir.model']._get_id('aypatech_ai.session'),
                'state': 'ai_session',
            }
        )
    ...
    def test_captured_context_keeps_json_safe_values_only(self):
        session = self.env['aypatech_ai.session'].create({'name': 'context'})
    ...
    def test_only_running_and_compacting_sessions_are_pending(self):
        ...
        for state in ('new', 'running', 'compacting', 'waiting', 'stopped', 'error'):
            session = self.env['aypatech_ai.session'].create({'name': f'state-{state}'})
            session.state = state
            sessions[state] = session
        self.env.flush_all()
        pending = set(self.env['aypatech_ai.session']._find_pending_session_ids(limit=100))
    ...
    def test_pending_lookup_ignores_session_ownership(self):
        ...
        self.env.flush_all()
        pending = self.env['aypatech_ai.session']._find_pending_session_ids(limit=100)
        self.assertIn(session.id, pending)

    def test_pending_lookup_is_capped_by_the_active_worker_count(self):
        crons = self.env['aypatech_ai.session']._session_worker_crons()
        self.assertTrue(crons)
        crons.write({'active': False})
        for index in range(3):
            self._pending_session(f'capped-{index}')
        self.env.flush_all()
        self.assertEqual(self.env['aypatech_ai.session']._active_worker_count(), 1)
        self.assertEqual(len(self.env['aypatech_ai.session']._find_pending_session_ids()), 1)
        crons[:2].write({'active': True})
        self.assertEqual(self.env['aypatech_ai.session']._active_worker_count(), 2)
        self.assertEqual(len(self.env['aypatech_ai.session']._find_pending_session_ids()), 2)
    ...
    def test_marking_a_session_in_error_closes_it_out(self):
        ...
        with self.enter_registry_test_mode():
            self.env['aypatech_ai.session']._mark_session_error(session.id, 'provider down')
        self.assertEqual(session.state, 'error')
        self.assertEqual(session.error_message, 'provider down')
        self.assertNotIn(
            session.id,
            self.env['aypatech_ai.session']._find_pending_session_ids(limit=100),
        )
        outputs = [
            item
    ...
    def test_cron_runs_a_queued_session_to_completion(self):
        ...
        with (
            ...
            self.env['aypatech_ai.session']._cron_run_pending_sessions()
        self.assertEqual(session.state, 'done')
        self.assertEqual(session.last_text, 'worker answer')

    def test_cron_leaves_a_settled_session_alone(self):
        session = self.env['aypatech_ai.session'].create({'name': 'cron-idle'})
        session.write(
            {
                'state': 'done',
                'conversation': [
