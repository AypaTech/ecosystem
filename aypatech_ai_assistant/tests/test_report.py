...
class TestMcpPrintReport(common.TransactionCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.env['ir.ui.view'].create(
            {
                'type': 'qweb',
                'name': 'aypatech_ai_assistant.mcp_test_print_report',
                'key': 'aypatech_ai_assistant.mcp_test_print_report',
                'arch': (
                    '<t t-name="aypatech_ai_assistant.mcp_test_print_report">'
        ...
        cls.report = cls.env['ir.actions.report'].create(
            {
                ...
                'report_name': 'aypatech_ai_assistant.mcp_test_print_report',
    ...
    def test_print_by_report_name(self):
        result = self._call(
            'print_report',
            {
                'report_ref': 'aypatech_ai_assistant.mcp_test_print_report',
                'ids': [self.partner.id],
            },
        )
        decoded = base64.b64decode(result['content_base64']).decode()
    ...
    def test_print_by_xmlid_prefix(self):
        ...
        if not xmlid:
            self.env['ir.model.data'].create(
                {
                    'name': 'aypatech_mcp_test_action_print_report',
                    'module': 'aypatech_mcp',
                    'model': 'ir.actions.report',
                    'res_id': self.report.id,
                },
            )
            xmlid = 'aypatech_ai_assistant.mcp_aypatech_mcp_test_action_print_report'
        result = self._call(
            'print_report',
            {
                'report_ref': xmlid,
    ...
    def test_print_unknown_xmlid_raises(self):
        with self.assertRaises(UserError):
            self._call(
                'print_report',
                {
                    'report_ref': 'aypatech_ai_assistant.mcp_nonexistent_xmlid',
                    'ids': [self.partner.id],
                },
            )
