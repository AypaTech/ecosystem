...
class TestSessionLimits(AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.cost_model = cls.env['aypatech_ai.model'].create(
            {
                'name': 'Limits',
    ...
    def _session(self, name: str = 'limits') -> models.Model:
        ...
        return self.env['aypatech_ai.session'].create({'name': name, 'agent_id': False})
    ...
    def test_cost_notice_warns_once_the_spend_crosses_the_ratio(self):
        self.env['ir.config_parameter'].sudo().set_str(
            'aypatech_ai_assistant.ai_turn_cost_limit', '1.0'
        )
        session = self._session()
