...
from odoo.addons.aypatech_ai_assistant.ai.tools import ADVISORY_LOCK_NAMESPACE
...
class TestAiSessionQueue(AITestCommon):
    ...
    def test_drain_merges_queued_messages_into_one_turn(self):
        ...
        self.assertEqual(
            [
                block['attachment_id']
                for block in entry['content']
                if block['type'] == 'aypatech_ai_attachment'
            ],
            [first.id, second.id],
        )
    ...
    def test_stop_compact_cancels_the_streaming_progress_event(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stop-compacting'})
    ...
    def test_stop_compact_keeps_a_finished_progress_event_intact(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stop-after-summary'})
        event = session._begin_compact_progress(auto=False)
    ...
    def test_append_event_allocates_gap_free_sequences(self):
        session = self.env['aypatech_ai.session'].create({'name': 'sequence-alloc'})
    ...
    def test_append_event_sequences_are_scoped_per_session(self):
        first = self.env['aypatech_ai.session'].create({'name': 'sequence-first'})
        second = self.env['aypatech_ai.session'].create({'name': 'sequence-second'})
    ...
    def test_append_event_retries_after_a_unique_violation(self):
        session = self.env['aypatech_ai.session'].create({'name': 'sequence-retry'})
        session._append_event({'kind': 'note', 'index': 0})
        event_model = type(self.env['aypatech_ai.session.event'])
    ...
    def test_submit_client_result_rejected_while_the_session_lock_is_held(self):
        session = self.env['aypatech_ai.session'].create({'name': 'busy-submit'})
        with (
