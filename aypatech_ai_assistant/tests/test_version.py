...
from odoo.addons.aypatech_ai_assistant.tests.common_mcp import MCPHttpCase
from odoo.addons.aypatech_ai_assistant.mcp.tools import common as mcp_common
from odoo.addons.aypatech_ai_assistant.mcp.tools import protocol, version
...
class TestOriginValidation(MCPHttpCase):
    ...
    def test_a_configured_origin_is_served(self):
        self.env['ir.config_parameter'].sudo().set_param(
            'aypatech_ai_assistant.mcp_allowed_origins',
    ...
    def test_a_rejected_origin_is_audited(self):
        log_model = self.env['aypatech_mcp.log']
    ...
    def test_the_delete_route_checks_the_origin(self):
        ...
        self.assertEqual(response.status_code, 403)
        session = self.env['aypatech_mcp.session'].search(
            [('session_id', '=', session_id)],
        )
