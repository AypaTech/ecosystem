...
class TestMcpKeyLifecycle(common.TransactionCase):
    ...
    def test_wizard_mints_a_working_key_and_reveals_it_once(self):
        ...
        wizard = (
            self.env['aypatech_mcp.generate_key']
            .with_user(owner)
            .create(
                {
                    'name': 'Wizard Key',
        ...
        self.assertEqual(action['res_model'], 'aypatech_mcp.key.show')
    ...
    def test_wizard_rate_limit_defaults_to_the_configured_value(self):
        self.config.set_param('aypatech_ai_assistant.mcp_rate_limit_requests', '17')
        wizard = self.env['aypatech_mcp.generate_key'].create({'name': 'Default Limit'})
    ...
    def test_generate_playground_key_uses_the_configured_rate_limit(self):
        self.config.set_param('aypatech_ai_assistant.mcp_rate_limit_requests', '7')
