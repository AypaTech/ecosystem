...
from odoo.addons.aypatech_ai_assistant.mcp.core.tool import get_tool_index
from odoo.addons.aypatech_ai_assistant.mcp.tools.encoder import encode_request
from odoo.addons.aypatech_ai_assistant.mcp.tools.exception import MCPScopeDenied
...
class TestMcpIntegration(common.TransactionCase):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.key_model = cls.env['aypatech_mcp.key']
        cls.tool_model = cls.env['aypatech_mcp.tool']
        cls.session_model = cls.env['aypatech_mcp.session']
        cls.notification_model = cls.env['aypatech_mcp.notification']
    ...
    def test_encoder_limits_attribute_size(self):
        old = config.options.get('aypatech_logging_attribute_limit')
        config['aypatech_logging_attribute_limit'] = 50
        try:
            ...
            if old is None:
                config.options.pop('aypatech_logging_attribute_limit', None)
            else:
                config['aypatech_logging_attribute_limit'] = old

    def test_encoder_limits_content_size(self):
        old = config.options.get('aypatech_logging_content_limit')
        config['aypatech_logging_content_limit'] = 200
        try:
            ...
            if old is None:
                config.options.pop('aypatech_logging_content_limit', None)
            else:
                config['aypatech_logging_content_limit'] = old
