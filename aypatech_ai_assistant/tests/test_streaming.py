...
from odoo.addons.aypatech_ai_assistant.ai.tools import StreamCancelled, TurnSuperseded
...
class TestAiStreaming(AITestCommon):
    ...
    def _capture_published(self) -> Iterator[list]:
        ...
        published = []
        session_cls = type(self.env['aypatech_ai.session'])
    ...
    def test_text_deltas_are_coalesced_into_the_published_answer(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-text'})
    ...
    def test_reasoning_deltas_stay_out_of_the_visible_answer(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-reasoning'})
    ...
    def test_tool_deltas_stream_the_tool_block_after_the_text(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-tools'})
    ...
    def test_cancel_mid_stream_aborts_and_persists_the_partial_once(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-cancel'})
    ...
    def test_second_cancel_does_not_duplicate_the_partial(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-recancel'})
    ...
    def test_cancel_with_empty_buffer_persists_no_assistant_message(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-empty'})
        session.write({'state': 'stopped'})
    ...
    def test_a_turn_replaced_mid_stream_stops_the_worker(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-superseded'})
    ...
    def test_a_stopped_session_is_not_reported_as_superseded(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-stopped'})
    ...
    def test_the_turn_being_streamed_is_left_alone(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-current'})
        session.write({'state': 'running', 'turn_seq': 4})
        session._check_cancelled({'turn': 4, 'last_state_check': 0})

    def test_a_replaced_turn_keeps_its_half_answer_to_itself(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-dropped'})
    ...
    def test_asking_again_after_a_stop_starts_a_later_turn(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-counter'})
    ...
    def test_a_caller_outside_a_turn_is_never_superseded(self):
        session = self.env['aypatech_ai.session'].create({'name': 'stream-no-turn'})
    ...
    @mute_logger('odoo.addons.aypatech_ai_assistant.ai.providers.base')
