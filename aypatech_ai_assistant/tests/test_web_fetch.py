...
from odoo.addons.aypatech_ai_assistant.tests.common import HTML_PAGE, AITestCommon, html_result
from odoo.addons.aypatech_ai_assistant.ai.tools.url_fetch import WEB_FETCH_MAX_CHARS, FetchResult
...
class TestWebFetchTool(AITestCommon):
    ...
    def test_success_returns_web_source_descriptor(self):
        with patch('odoo.addons.aypatech_ai_assistant.ai.mcp.web.fetch_url', return_value=html_result()):
            result = self.env['aypatech_mcp.mixin']._mcp_web_fetch(
    ...
    def test_pagination_truncates_with_marker(self):
        ...
        with patch('odoo.addons.aypatech_ai_assistant.ai.mcp.web.fetch_url', return_value=big):
            result = self.env['aypatech_mcp.mixin']._mcp_web_fetch(
                url='https://example.com/big',
                max_chars=100,
            )
    ...
    def test_fetch_error_returned_not_raised(self):
        with patch(
            'odoo.addons.aypatech_ai_assistant.ai.mcp.web.fetch_url',
