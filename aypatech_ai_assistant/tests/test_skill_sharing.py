...
@tagged('post_install', '-at_install', 'aypatech_ai_skills', 'sharing')
class TestSkillSharing(TransactionCase):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.Skill = cls.env['aypatech_ai.skill']
    ...
    def test_read_rule_matches_visibility_domain(self):
        access = self.env.ref('aypatech_ai_assistant.skills_access_aypatech_ai_skill_user_r')
        rule_domain = safe_eval(access.domain, {'user': self.user_owner})
    ...
    def test_create_relinks_pending_attachment_to_skill(self):
        ...
        self.assertEqual(attachment.res_model, 'aypatech_ai.skill')
