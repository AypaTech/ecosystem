...
class TestShare(TransactionCase):
    ...
    def test_the_shared_space_collects_it_for_the_reader(self):
        space = self.env.ref('aypatech_ai_assistant.ai_space_shared')
        found = (
            self.env['aypatech_ai.session']
            .with_user(self.reader)
    ...
    def test_the_shared_space_keeps_a_chat_the_owner_filed_away(self):
        space = self.env['aypatech_ai.space'].with_user(self.owner).create({'name': 'Mine'})
        self.session.with_user(self.owner).write({'space_id': space.id})
        shared = self.env.ref('aypatech_ai_assistant.ai_space_shared')
        found = (
            self.env['aypatech_ai.session']
            .with_user(self.reader)
    ...
    def test_filing_a_chat_keeps_it_for_the_people_it_is_shared_with(self):
        space = self.env['aypatech_ai.space'].with_user(self.owner).create({'name': 'Mine'})
        own = self.env['aypatech_ai.session'].with_user(self.owner).create({'name': 'Own'})
        own.write({'share_user_ids': [(6, 0, self.stranger.ids)]})
        shared = self.env.ref('aypatech_ai_assistant.ai_space_shared')
        domain = shared.with_user(self.stranger)._session_domain()
        Session = self.env['aypatech_ai.session'].with_user(self.stranger)
    ...
    def test_the_owner_still_finds_a_chat_they_shared_out(self):
        general = (
            self.env['aypatech_ai.space'].with_user(self.owner)._unclaimed_session_domain()
        )
        found = (
            self.env['aypatech_ai.session']
            .with_user(self.owner)
    ...
    def test_a_chat_shared_with_me_leaves_my_general_list(self):
        general = (
            self.env['aypatech_ai.space'].with_user(self.reader)._unclaimed_session_domain()
        )
        found = self.env['aypatech_ai.session'].with_user(self.reader).search(general)
        self.assertNotIn(self.session.id, found.ids)
