...
from .common_chatter import ChatterTestCommon


@tagged('post_install', '-at_install', 'aypatech_ai_chatter')
class TestCompose(ChatterTestCommon):
    ...
    def setUp(self) -> None:
        ...
        super().setUp()
        self.Session = self.env['aypatech_ai.session']
    ...
    def test_the_helper_is_collected_by_its_own_space(self):
        space = self.env.ref('aypatech_ai_assistant.chatter_space_writing')
    ...
    def test_the_record_space_leaves_the_helpers_alone(self):
        space = self.env.ref('aypatech_ai_assistant.chatter_space_records')
        collected = self.Session.search(space._session_domain())
    ...
    def test_the_helper_says_so_when_no_agent_can_answer(self):
        self.env['aypatech_ai.agent'].search([]).write({'active': False})
    ...
    def test_the_chips_a_composer_offers_are_skills(self):
        offered = self.env['aypatech_ai.skill'].fetch_skills('composer')
    ...
    def test_a_writing_chip_must_say_which_group_it_belongs_to(self):
        with self.assertRaises(ValidationError):
            self.env['aypatech_ai.skill'].create(
                {
    ...
    def test_a_chat_skill_needs_no_group(self):
        skill = self.env['aypatech_ai.skill'].create(
            {
                'name': 'chat_ungrouped',
                'skill_type': 'chat',
    ...
    def test_an_archived_chip_is_offered_to_nobody(self):
        skill = self.env.ref('aypatech_ai_assistant.chatter_skill_compose_shorten')
        skill.active = False
        offered = self.env['aypatech_ai.skill'].fetch_skills('composer')
