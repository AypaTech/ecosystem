...
from odoo.addons.aypatech_ai_assistant.mcp.core.tool import invalidate_registry_cache, mcp_tool
...
def _mcp_test_ctx_probe(self):
    return {'flag': self.env.context.get('aypatech_mcp_probe')}
...
class TestMcpDispatch(common.TransactionCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.mixin_cls = type(cls.env['aypatech_mcp.mixin'])
    ...
    def test_context_override_reaches_python_tool(self):
        text, _info = self.tool_model._call(
            'mcp_test_ctx_probe',
            {'context': {'aypatech_mcp_probe': 'here'}},
            self.env,
        )
    ...
    def test_context_override_reaches_db_tool(self):
        tool = self.tool_model.sudo().create(
            {
                ...
                'description': 'Return the probe flag from env.context.',
                'category': 'read',
                'code': "result = {'flag': env.context.get('aypatech_mcp_probe')}\n",
        ...
        try:
            text, _info = self.tool_model._call(
                ...
                {'context': {'aypatech_mcp_probe': 'db_here'}},
                self.env,
            )
    ...
    def test_context_override_does_not_mutate_caller_env(self):
        self.tool_model._call(
            'mcp_test_ctx_probe',
            {'context': {'aypatech_mcp_probe': 'temp'}},
            self.env,
        )
        self.assertIsNone(self.env.context.get('aypatech_mcp_probe'))
