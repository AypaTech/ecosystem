...
from odoo.addons.aypatech_ai_assistant.tests.common import AITestCommon, ToolCatalogMixin
from odoo.addons.aypatech_ai_assistant.ai.tools.call import (
...
class TestToolLazy(ToolCatalogMixin, AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls.session = cls.env['aypatech_ai.session'].create({'name': 'Lazy session'})
        cls.catalog = [
            {
                'name': 'list_models',
    ...
    def test_tool_load_exact_match_wins_over_prefix_fallback(self):
        ...
        session = self.env['aypatech_ai.session'].create({'name': 'Dotted catalog'})
        with patch.object(
            type(self.env['aypatech_mcp.tool']),
            'get_tools',
            autospec=True,
    ...
    def test_agent_get_essential_tool_names_defaults(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Defaults'})
    ...
    def test_default_essential_tool_names_is_overridable(self):
        defaults = self.env['aypatech_ai.agent']._get_default_essential_tool_names()
        for name in ('list_models', 'search_count', 'read_records'):
            self.assertIn(name, defaults)
    ...
    def test_deferred_loop_executes_target_tool_like_eager(self):
        ...
        session_lazy = self.env['aypatech_ai.session'].create(
            {
                'name': 'Lazy parity',
                'agent_id': agent.id,
            }
        )
        session_eager = self.env['aypatech_ai.session'].create(
            {
                'name': 'Eager parity',
                'agent_id': agent.id,
            }
