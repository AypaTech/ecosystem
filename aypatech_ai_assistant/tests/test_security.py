...
class TestAiSecurity(TransactionCase):
    ...
    def test_user_sees_own_sessions_only(self):
        ...
        self.env['aypatech_ai.session'].with_user(self.user_b).create({'name': 'B session'})
        visible_to_a = (
            self.env['aypatech_ai.session']
            .with_user(self.user_a)
    ...
    def test_manager_sees_all_sessions(self):
        self.env['aypatech_ai.session'].with_user(self.user_a).create({'name': 'A session'})
        self.env['aypatech_ai.session'].with_user(self.user_b).create({'name': 'B session'})
        visible = (
            self.env['aypatech_ai.session']
    ...
    def test_agent_write_requires_manager(self):
        agent = self.env['aypatech_ai.agent'].create({'name': 'Read only'})
    ...
    def test_rate_limit_blocks_excessive_creates(self):
        provider = self.env.ref('aypatech_ai_assistant.ai_provider_openai').sudo()
        provider.write({'rate_limit': 2})
        self.env.company.default_ai_provider_id = provider
        Session = self.env['aypatech_ai.session'].with_user(self.user_a)
