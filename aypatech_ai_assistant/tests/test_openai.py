...
from odoo.addons.aypatech_ai_assistant.ai.tools import build_tool_call_output
...
class TestAiOpenAIProvider(AITestCommon):
    ...
    def test_request_responses_sends_prompt_cache_key(self):
        ...
        with patch.object(requests.Session, 'post', side_effect=fake_post):
            self.provider._request_responses(inputs=[], cache_key='aypatech_ai.session:42')
        self.assertEqual(captured['body']['prompt_cache_key'], 'aypatech_ai.session:42')
    ...
    def test_rejected_effort_is_stripped_and_served(self):
        record = self.env.ref('aypatech_ai_assistant.model_gpt_5_mini')
        bodies = []
