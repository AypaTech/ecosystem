...
class TestSessionBus(TransactionCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.session = (
            cls.env['aypatech_ai.session'].with_user(cls.owner).create({'name': 'Streamed'})
        )
    ...
    def _session_channels(self, channels: list) -> list:
        ...
        return [
            ...
            if isinstance(channel, models.Model) and channel._name == 'aypatech_ai.session'
        ]
    ...
    def test_the_transcript_rides_the_session_channel(self):
        ...
        self.assertEqual(self._targets(captured, 'aypatech_ai_assistant.ai_event'), [self.session])

    def test_the_sidebar_state_reaches_the_owner_and_the_readers(self):
        with self._captured() as captured:
            self.session._publish_event('rename', {'name': 'Renamed'})
        self.assertEqual(
            set(self._targets(captured, 'aypatech_ai_assistant.ai_session_state')),
            {self.owner.partner_id, self.reader.partner_id},
        )

    def test_a_deletion_reaches_the_readers_too(self):
        session = self.env['aypatech_ai.session'].with_user(self.owner).create({'name': 'X'})
        session.sudo().write({'share_user_ids': [(6, 0, self.reader.ids)]})
        with self._captured() as captured:
            session.sudo().unlink()
        self.assertIn(
            self.reader.partner_id, self._targets(captured, 'aypatech_ai_assistant.ai_session_state')
        )
    ...
    def test_a_chat_that_does_not_exist_grants_nothing(self):
        channels = self._channels(self.owner, ['aypatech_ai_assistant.ai_session_999999999'])
    ...
    def test_dropping_a_reader_takes_the_chat_off_their_list(self):
        ...
        dropped = [
            ...
            if ntype == 'aypatech_ai_assistant.ai_session_state' and target == self.reader.partner_id
        ]
    ...
    def test_adding_a_reader_puts_the_chat_on_their_list(self):
        ...
        added = [
            message
            for target, ntype, message in captured
            if ntype == 'aypatech_ai_assistant.ai_session_state' and target == self.stranger.partner_id
        ]
