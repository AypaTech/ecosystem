...
from odoo.addons.aypatech_ai_assistant.ai.models.session import AISession
...
class TestSpace(AITestCommon):
    ...
    def _space(self, name: str, **values) -> models.Model:
        ...
        return self.env['aypatech_ai.space'].create({'name': name, **values})

    def _session(self, name: str, **values) -> models.Model:
        """Create a chat session owned by the current user."""
        return self.env['aypatech_ai.session'].create({'name': name, **values})
    ...
    def test_fetch_spaces_describes_a_personal_space(self):
        ...
        entry = next(
            row
            for row in self.env['aypatech_ai.space'].fetch_spaces()
            if row['id'] == space.id
        )
    ...
    def test_count_sessions_sorts_ids_into_personal_spaces(self):
        ...
        counts = self.env['aypatech_ai.space'].count_sessions(
    ...
    def test_count_sessions_sorts_ids_into_system_spaces(self):
        ...
        counts = self.env['aypatech_ai.space'].count_sessions([session.id])
    ...
    def test_count_sessions_ignores_sessions_outside_the_given_ids(self):
        ...
        counts = self.env['aypatech_ai.space'].count_sessions([skipped.id])
        self.assertEqual(counts[str(space.id)], 1)

    def test_counting_personal_spaces_does_not_query_per_space(self):
        spaces = self.env['aypatech_ai.space']
    ...
    def test_count_sessions_without_ids_is_empty(self):
        ...
        self.assertEqual(self.env['aypatech_ai.space'].count_sessions([]), {})
    ...
    def test_the_badge_payload_carries_the_per_space_counts(self):
        ...
        payload = self.env['aypatech_ai.session'].notification_badge()
    ...
    def test_fetch_spaces_marks_a_system_space(self):
        ...
        collected = self.env['aypatech_ai.session'].search(entry['session_domain'])
    ...
    def test_fetch_spaces_hides_other_users_spaces_from_an_admin(self):
        ...
        listed = {row['id'] for row in self.env['aypatech_ai.space'].fetch_spaces()}
    ...
    def test_reorder_stores_the_given_order(self):
        ...
        self.env['aypatech_ai.space'].reorder([third.id, first.id, second.id])
        ordered = self.env['aypatech_ai.space'].search(
    ...
    def test_a_user_only_sees_their_own_and_the_system_spaces(self):
        ...
        visible = self.env['aypatech_ai.space'].with_user(other).search([])
    ...
    def test_opening_a_space_lists_the_chats_it_collects(self):
        ...
        listed = self.env['aypatech_ai.session'].search(action['domain'])
    ...
    def test_the_instructions_are_handed_over_unrendered(self):
        raw = (
            'Secret: {{ env["ir.config_parameter"].sudo().get_str("database.uuid") }}'
        )
