...
class TestMcpAclIsolation(common.TransactionCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.log_model = cls.env['aypatech_mcp.log']
        ...
        cls.env['ir.ui.view'].create(
            {
                'type': 'qweb',
                'name': 'aypatech_ai_assistant.mcp_test_acl_report',
                'key': 'aypatech_ai_assistant.mcp_test_acl_report',
                'arch': (
                    '<t t-name="aypatech_ai_assistant.mcp_test_acl_report">'
        ...
        cls.report = cls.env['ir.actions.report'].create(
            {
                'name': 'MCP ACL Report',
                'report_name': 'aypatech_ai_assistant.mcp_test_acl_report',
    ...
    def _make_notification(
        ...
        session = self.env['aypatech_mcp.session'].create({'user_id': user.id})
    ...
    def test_call_method_skips_the_record_hook_for_api_model_methods(self):
        ...
        mixin_cls = type(self.env['aypatech_mcp.mixin'])
    ...
    def test_get_access_rights_reads_the_rule_list_with_sudo(self):
        ...
        with self.assertRaises(AccessError):
            self.env['ir.access'].with_user(self.user).check_access('read')
