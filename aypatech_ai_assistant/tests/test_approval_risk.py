...
class TestApprovalRiskPredicate(AITestCommon):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        cls._mark_sensitive('res.partner', 'aypatech_ai.session')
    ...
    def test_delete_on_sensitive_model_is_risky(self):
        risk = self.env['aypatech_ai.approval']._assess_risk(
            'delete_records',
            {
                'model': 'res.partner',
                'ids': [1, 2],
    ...
    def test_signature_stable_for_same_tool_and_model(self):
        a = self.env['aypatech_ai.approval']._assess_risk(
            'update_records',
            {
                'model': 'res.partner',
                'ids': [1],
        ...
        b = self.env['aypatech_ai.approval']._assess_risk(
            'update_records',
            {
                'model': 'res.partner',
                'ids': [9],
