...
class TestClientToolSeam(AITestCommon):
    ...
    def _new_session(self, name: str = 'client') -> models.Model:
        ...
        return self.env['aypatech_ai.session'].create({'name': name})
    ...
    def test_adjust_search_server_body_raises(self):
        with self.assertRaises(UserError):
            self.env['aypatech_mcp.mixin']._mcp_adjust_search()
    ...
    def test_stale_client_action_swept(self):
        ...
        with provider, self._as_client_tool('browser_click'):
            ...
            self.env['aypatech_ai.session']._sweep_stale_client_actions()
    ...
    def test_the_timeout_can_be_set_from_the_settings(self):
        ...
        settings.execute()
        self.assertEqual(self.env['aypatech_ai.session']._client_action_timeout(), 45)

    def test_timeout_param_falls_back_on_invalid_value(self):
        self.env['ir.config_parameter'].sudo().set_param(
            'aypatech_ai_assistant.ai_client_action_timeout', 'off'
        )
        self.assertEqual(self.env['aypatech_ai.session']._client_action_timeout(), 600)

    def test_sweep_disabled_by_param(self):
        session = self._new_session('disabled')
        self.env['ir.config_parameter'].sudo().set_param(
            'aypatech_ai_assistant.ai_client_action_timeout', '0'
        )
        provider = self._script_provider(
            [
                self._tool_payload('browser_click', {'ref': 'e1'}, 'c0'),
