...
class TestMigration(ScheduleTestCommon):
    ...
    def _load_migration(self, version: str, phase: str) -> Callable:
        ...
        spec = importlib.util.spec_from_file_location(
            'aypatech_ai_schedule_migration_%s_%s' % (version.replace('.', '_'), phase),
            path,
        )
    ...
    def _strip_owned_records(self, schedule: models.BaseModel) -> None:
        ...
        self.env.flush_all()
        self.env.cr.execute(
            'UPDATE aypatech_ai_schedule SET action_server_id = NULL, cron_id = NULL '
            'WHERE id = %s',
            (schedule.id,),
        )
    ...
    def _write_nextcall_snapshot(
        ...
        self.addCleanup(
            self.env.cr.execute,
            'DROP TABLE IF EXISTS _aypatech_ai_schedule_nextcall_snapshot',
        )
        self.env.cr.execute("""
            DROP TABLE IF EXISTS _aypatech_ai_schedule_nextcall_snapshot;
            CREATE TABLE _aypatech_ai_schedule_nextcall_snapshot (
                id integer, next_call timestamp, last_call timestamp
            );
        """)
        self.env.cr.execute(
            'INSERT INTO _aypatech_ai_schedule_nextcall_snapshot VALUES (%s, %s, %s)',
