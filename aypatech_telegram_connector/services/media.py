...
class TelegramMediaService:
    ...
    def download_limit(self):
        ...
        custom_server = self.env["ir.config_parameter"].sudo().get_param("telegram.api_base_url")
        return limit if custom_server else min(limit, BOT_API_DOWNLOAD_LIMIT)

    def upload_limit(self):
        custom = self.env["ir.config_parameter"].sudo().get_param("telegram.max_upload_mb")
        return int(custom) * MB if custom and custom.isdigit() else BOT_API_UPLOAD_LIMIT
    ...
    def send_attachment(self, chat_id, attachment, caption=None):
        ...
        file_tuple = (attachment.name or "file", attachment.raw or b"", attachment.mimetype or "application/octet-stream")
