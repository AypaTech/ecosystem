...
class DiscussChannel(models.Model):
    ...
    def _message_post_after_hook(self, message):
        res = super()._message_post_after_hook(message)
        for channel in self:
    ...
    def _types_allowing_unfollow(self):
        return super()._types_allowing_unfollow() + ["telegram"]
    ...
    def _store_channel_fields(self, res: Store.FieldList):
        ...
        res.attr(
            ...
            lambda channel: channel.telegram_conversation_id.id,
            predicate=_is_telegram,
        )
        # sudo: telegram.conversation - only the customer partner id of an accessible channel
        res.attr(
            "telegram_partner_id",
            lambda channel: channel.telegram_conversation_id.sudo().partner_id.id,
            predicate=_is_telegram,
        )
