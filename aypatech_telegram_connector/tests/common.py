...
class TelegramCase(TransactionCase):
    ...
    def tracked_labels(self, records):
        """Labels of the fields tracked on ``records`` (20 renders tracking into the body)."""
        return set(re.findall(r"<i>\((.*?)\)</i>", " ".join(str(m.body) for m in records.message_ids)))
